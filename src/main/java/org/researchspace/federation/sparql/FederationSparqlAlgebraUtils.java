...
public class FederationSparqlAlgebraUtils {
    ...
    private static class InputVarCollectorVisitor extends AbstractQueryModelVisitor<RepositoryException> {
    ...
    private static class OutputVarCollectorVisitor extends AbstractQueryModelVisitor<RepositoryException> {
    ...
    private static class OwnerScanner extends AbstractQueryModelVisitor<RepositoryException> {
        ...
        private OwnerScanner(Map<IRI, ? extends Repository> serviceMappings, RepositoryConnection mainMember) {
    ...
    /**
     * Given a {@link TupleExpr} and a list of {@link OrderElem}s, selects only
     * those order elements, which would be used for sorting the output of the
     * {@link TupleExpr} (i.e., those which have variables in common). Used when
     * processing the top-k queries with ORDER BY to "push down" the order
     * expression in the query tree.
     * 
     * @param tupleExpr
     * @param orderElements
     * @return
     */
    public static List<OrderElem> getRelevantOrderElements(TupleExpr tupleExpr, List<OrderElem> orderElements) {
    ...
    public static Set<String> getInputBindingNames(TupleExpr tupleExpr) {
        return getInputVars(tupleExpr).stream().filter(var -> !var.hasValue()).map(var -> var.getName())
    ...
    public static Set<String> getOutputBindingNames(TupleExpr tupleExpr) {
        OutputVarCollectorVisitor visitor = new OutputVarCollectorVisitor();
        tupleExpr.visit(visitor);
        return getOutputVars(tupleExpr).stream().filter(var -> !var.hasValue()).map(var -> var.getName())
                .collect(Collectors.toSet());
    }

    public static boolean executeOnSingleOwnerWithoutOptimization(TupleExpr tupleExpr, MpFederation mpFederation,
            RepositoryConnection defaultMemberConnection) {
    ...
    /**
     * Among the federation members, returns a single repository connection which
     * can evaluate a given {@link TupleExpr} (if avaiable). Used to determine
     * whether the query needs to be processed by the federation or can be sent as a
     * whole to one endpoint.
     * 
     * @param arg
     ...
     */
    public static RepositoryConnection getSingleOwner(TupleExpr arg, Map<IRI, ? extends Repository> serviceMappings,
            RepositoryConnection mainMember) throws RepositoryException {
    ...
    public static boolean hasUnsatisfiedDependencies(TupleExpr arg, BindingSet bs) {
        ...
        if (bs != null) {
            unsatisfiedInputs = unsatisfiedInputs.stream().filter(var -> !bs.hasBinding(var.getName()))
                    .collect(Collectors.toList());
        }
