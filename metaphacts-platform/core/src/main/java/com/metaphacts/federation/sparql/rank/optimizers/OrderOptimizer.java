...
import com.metaphacts.federation.sparql.rank.algebra.RankedNaryUnion;
 ...
 *
 */
public class OrderOptimizer extends AbstractQueryModelVisitor<RepositoryException>
        implements QueryOptimizer {
    
    private boolean insideService = false;

    protected class OrderTupleExprHandlerVisitor
    ...
    protected boolean handleTupleExpression(final Order topOrder, TupleExpr exp,
            boolean insideService) {
    ...
    protected QueryModelNode getRightPositionInUnaryOperatorSequence(TupleExpr currentNode) {
        ...
        if (currentNode.getParentNode() instanceof Projection) {
            return getRightPositionInUnaryOperatorSequence(
                    (Projection) currentNode.getParentNode());
        }
        return currentNode;
    }

    protected boolean handleNTuple(final Order topOrder, AbstractNaryTupleOperator nTuple,
            boolean insideService) {
        ...
        for (TupleExpr arg : args) {
            ...
            relevantOrderElements = FederationSparqlAlgebraUtils.getRelevantOrderElements(arg,
        ...
        if (replaceWithRankTuple) {
            ...
            if (nTuple instanceof NaryJoin) {
                nRankTuple = new RankedNaryJoin(new ArrayList<TupleExpr>(sortedArgs.keySet()),
                        unsortedArgs, topOrder.getElements());
            } else if (nTuple instanceof NaryUnion) {
                nRankTuple = new RankedNaryUnion(new ArrayList<TupleExpr>(sortedArgs.keySet()),
                        unsortedArgs, topOrder.getElements());
            }
            if (nRankTuple != null) {
