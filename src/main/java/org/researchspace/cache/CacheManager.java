...
public class CacheManager {
    ...
    public void register(PlatformCache cache) {
        ...
        if (instances.containsKey(cacheId)) {
            throw new IllegalStateException(String.format("CacheManager has already an cache with ID '%s'.", cacheId));
        } else {
    ...
    public void invalidateResources(Set<IRI> resources) {
        if (logger.isDebugEnabled()) {
            logger.debug("Invalidating the following resources: {}",
                    String.join(", ", resources.stream().map(IRI::toString).collect(toList())));
        }
        for (PlatformCache cache : instances.values()) {
    ...
    protected String getCacheSpec(String cacheId) {
        ...
        if (org.researchspace.security.PlatformSecurityManager.AUTH_CACHE_NAME.equals(cacheId)) {
