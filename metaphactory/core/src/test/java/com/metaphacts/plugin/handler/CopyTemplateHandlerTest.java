 ...
 *
 */
public class CopyTemplateHandlerTest extends InstallationHandlerTest {


    @Test
    public void testCopyNoneExistingAndSkippingExisting() throws IOException{
        ...
        CopyTemplateHandler handler = new CopyTemplateHandler(config, appDir.toPath());
        handler.install();
        
        // this should still exist
        assertTrue(platformPageTemplateFile.exists());
        // and it should be still the old content in the platform page template i.e. not overwritten
        assertEquals(
                Lists.<String>newArrayList("Platform Page Template"), 
        ...
        assertThat(appender.getLogMessages(), hasItems(
                "File \""+ pluginPageTemplateFile.getAbsolutePath() +"\" already exists in \""+ getPlatformTemplateFolder() +"\". Skipping.",
    ...
    protected List<Class<? extends PluginBaseInstallationHandler>> getClassesToLog() {
        return Lists.newArrayList(CopyTemplateHandler.class);
    }
