        ...
        queries.stream().forEach(resource -> {
            ...
            while (matcher.find()) {
                ...
                res.stream()
                        .filter(
                                reso -> {
                                    String tmp = Models.getPropertyString(model, reso, SPL.PREDICATE_PROPERTY)
                                            .orElse("");
                                    return StringUtils.equals(MpRepositoryVocabulary.NAMESPACE
                                            .concat(SQL_PARAMETER_DELIMITER).concat(varName), tmp);
                                })
                        .forEach(
                                reso -> {
                                    IRI typeIri = Models.getPropertyIRI(model, reso, SPL.VALUETYPE_PROPERTY)
                                            .orElse(null);
