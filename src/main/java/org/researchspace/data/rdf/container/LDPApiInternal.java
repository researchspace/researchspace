...
/**
 * {@link LDAPApiInterface} implementation that doesn't check for any
 * permissions. Should be used only by internal services. All external requests
 * should be channeled through {@link PermissionsAwareLDPApi}.
 *
...
public class LDPApiInternal extends LDPApi implements LDPApiInterface {
    ...
    /**
     * A method to save an already prepared model for a new LDP resource. It is
     * assumed that the IRI of the resource to be saved should be preserved "as is"
     * without artificially constructing a new one.
     *
     * @param graphFrom       a {@link PointedGraph} containing the model to be
     *                        saved and the IRI of the new LDP resource
    ...
    /**
     * A method to update an LDP resource using an already prepared model. It is
     * assumed that the IRI of the resource to be saved should be preserved "as is"
     * without artificially constructing a new one.
     *
     * @param graphFrom        a {@link PointedGraph} containing the model to be
     *                         saved. The resource serving as the pointer of the
     *                         {@link PointedGraph} will be replaced with
     *                         <code>resourceToUpdate</code>.
     * @param resourceToUpdate an existing LDP resource to be updated using
     *                         <code>graphFrom</code>.
     * @return
     */
    ...
    public List<LDPResource> importLDPResource(Model resource, Set<IRI> possibleContainers, Optional<IRI> containerIRI,
        ...
        return super.importLDPResource(resource, possibleContainers, containerIRI, unknownObjects, force, instanceBase,
                this::noneContainerCheck);
    }

    @Override
    public LDPResource copyLDPResource(Optional<String> slug, IRI uri, Optional<IRI> targetContainer,
            String instanceBase) {
        return super.copyLDPResource(slug, uri, targetContainer, instanceBase, this::noneResourceCheck,
                this::noneContainerCheck);
    }
