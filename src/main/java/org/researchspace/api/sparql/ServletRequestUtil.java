...
public class ServletRequestUtil {
    ...
    /**
     * Pretty logging of request parameters and request headers to supplied loggerif
     * logger is traced enabled.
     *
     * @param req
     * @param logger
     */
    ...
    /**
     * Finds the best matching mime type from the {@link HttpHeaders#ACCEPT} header
     * of the supplied {@link HttpServletRequest}.
     *
    ...
    public static Optional<String> getPreferredMIMEType(Collection<String> possibleMimeTypes, HttpServletRequest req) {
        if (possibleMimeTypes == null || possibleMimeTypes.isEmpty())
            return Optional.<String>empty();

        /*
         * Most clients will only send one "Accept" header field, for example: Accept:
         * text/csv;q=0.8,application/sparql-results+json;q=0.8,application/sparql-
         * results+xml However HTTP specification also allows to send multiple "Accept"
         * fields (that is, for example, what the sesame sparql client does): Accept:
         * text/csv;q=0.8 Accept: application/sparql-results+json;q=0.8 Accept:
         * application/sparql-results+xml As such we need to concatenate multiple
         * headers by comma, as input for {@link MIMEParse#bestMatch}.
         */
    ...
    /**
     * Extracts the MimeType string from the "Content-Type" header string without,
     * for example, charset.
     *
     * @param expectedContentType list of possible/expected mime types. If no list
     *                            is supplied, first element of the header string
     *                            will be returned (which is usually the content
     *                            type).
     * @param req
     * @return best matching content mime type string
     */
    public static Optional<String> getContentType(HttpServletRequest req,
            Optional<Collection<String>> expectedContentType) {
        if (StringUtils.isEmpty(req.getContentType())) {
            return Optional.<String>empty();
        }

        if (!expectedContentType.isPresent()) {
            for (HeaderElement e : BasicHeaderValueParser.parseElements(req.getContentType(),
