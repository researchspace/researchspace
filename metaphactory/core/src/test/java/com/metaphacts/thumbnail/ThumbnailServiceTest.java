...
public class ThumbnailServiceTest extends AbstractRepositoryBackedIntegrationTest {
    ...
    public void setup() throws Exception {
        config.getUiConfig().setParameter(
            "preferredThumbnails", String.format("<%s>", THUMBNAIL.stringValue()));
    }
    ...
    public void testFetchesThumbnailsSpecifiedAsLiteralOrIRI() throws Exception {
        ...
        Map<IRI, Optional<IRI>> thumbnails = service.getThumbnailIRIs(
            repositoryRule.getRepository(), Lists.newArrayList(FOO, BAR));
        Assert.assertTrue(thumbnails.get(FOO).isPresent() && thumbnails.get(FOO).get().equals(FOO_THUMBNAIL));
        Assert.assertTrue(thumbnails.get(BAR).isPresent() && thumbnails.get(BAR).get().equals(BAR_THUMBNAIL));
    }

    @Test
    public void testIgnoresMissingOrInvalidThumbnailIRIs() throws Exception {
        addStatements(Lists.newArrayList(
            vf.createStatement(QUX, THUMBNAIL, vf.createLiteral("not-an-iri"))
        ));
