...
tinymce.PluginManager.add('fullpage', function(editor) {
	...
	function setContent(evt) {
		...
		// Ignore raw updated if we already have a head, this will fix issues with undo/redo keeping the head/bart separate
		...
		// Fixed so new document/setContent('') doesn't remove existing header/barter except when it's in source code view
		if (content.length === 0 && !evt.source_view) {
			content = tinymce.trim(head) + '\n' + tinymce.trim(content) + '\n' + tinymce.trim(bart);
		}

		// Parse out head, body and barter
		...
		if (startPos != -1) {
			...
			bart = low(content.substring(endPos));
		} else {
