 ...
 * {@link org.apache.shiro.authz.permission.WildcardPermission} that
 * <ul>
 * <li>escapes ":" inside "<>", to make possible use of IRIs in SHIRO permission
 * strings. E.g my:permission:&lt;http://example.com&gt;:edit</li>
 * <li>supports arbitrary regex filters over permission string parts. As of now,
 * regex permissions have the format <code>regex(MY_REGULAR_EXPRESSION)</code>
...
public class WildcardPermission extends org.apache.shiro.authz.permission.WildcardPermission {
    ...
    protected String escapePatternIfNeeded(Pattern pattern, String wildcardString, Map<String, String> escapedMap,
    ...
    protected List<Set<String>> unescapeParts(List<Set<String>> escapedParts, Map<String, String> escapedMap) {
        ...
        // corruption (e.g., if "<urn;myurn>" gets replaced before
        // "regex(<urn;myurn>|<urn;otherurn>)",
        // corrupts the original string and leads to the "regex(...)" pattern not being
        // unescaped properly).
        List<String> keyList = Lists.newArrayList(escapedMap.keySet());
        keyList.sort((s1, s2) -> {
            return s1.length() > s2.length() ? -1 : (s1.length() == s2.length()) ? 0 : 1;
        });
        return escapedParts.stream().map(sets -> sets.stream().map(p -> {
    ...
    /**
     * We select the regex patterns defined in the permission string, compile them
     * immediately and store to avoid expensive regex pattern compiling during the
