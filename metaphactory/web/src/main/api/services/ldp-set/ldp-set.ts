...
export class SetService extends LdpService {
  ...
  reorderItems(setIri: Rdf.Iri, holders: Immutable.List<HolderWithItem>): Kefir.Property<void> {
    ...
    return Kefir.zip(
      holders.map(({holder, item}, index) => set.update(
        holder, createItemHolderGraph(holder, item, index)
      )).toArray()
...
function addSetItem(
  ...
  const graph = createItemHolderGraph(Rdf.iri(''), item, index);
  return set.addResource(graph).map(holder => ({holder, item}));
}

function createItemHolderGraph(holderIri: Rdf.Iri, itemIri: Rdf.Iri, index?: number) {
