...
/**
 * Represents an abstract storage of so-called objects which are BLOBs indexed
 * by {@link ObjectKind} and string IDs and have metadata associated with each
 * one.
 */
public interface ObjectStorage extends AutoCloseable {
    /**
     * @return {@code true} if it's possible to modify storage data through mutation
     *         methods {@link #appendObject}, {@link #deleteObject}, etc;
     *         {@code false} if storage is read-only
     */
    boolean isMutable();
     ...
     */
    Optional<ObjectRecord> getObject(StoragePath path, @Nullable String revision) throws StorageException;
    ...
    /**
     * Fetches records for objects with specified kind which matches specified path
     * prefix.
     *
    ...
    /**
     * Either: - adds new object if it doesn't exists yet; - adds new revision to
     * existing object; - replaces current object revision if storage doesn't
     * support versioning.
     *
     * @param path          target object path
     ...
     */
    ObjectRecord appendObject(StoragePath path, ObjectMetadata metadata, InputStream content, long contentLength)
            throws StorageException;

    /**
     * Deletes the specified object from storage. Should not throw an exception if
     ...
     */
    void deleteObject(StoragePath path, ObjectMetadata metadata) throws StorageException;
