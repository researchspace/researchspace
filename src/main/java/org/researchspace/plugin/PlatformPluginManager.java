...
public class PlatformPluginManager extends DefaultPluginManager {
    ...
    private static final Logger logger = LogManager.getLogger(PlatformPluginManager.class);
    ...
    public PlatformPluginManager() {
        ...
        if (this.getPluginsRoot().toFile().exists()) {
            ...
            // loaded
            logger.warn("App directory {} does not exist. No apps will be initialized.", this.getPluginsRoot());
        }
    }
    ...
    public static class Starter {
        @Inject
        public Starter(PlatformPluginManager pluginManager, Configuration config,
    ...
    private void startPlugins(Configuration config, MpJDBCDriverManager jdbcDriverManager) {
        ...
        for (final PlatformPlugin mphPlugin : asPlatformPlugins(this.getPlugins())) {
            ...
            if (mphPlugin.getClass().equals(PlatformPlugin.class)) {
                logger.info("Using default {} as main class.", PlatformPlugin.class.getName());
            } else {
                logger.info("Using custom platform plugin class extension {} as provided by the plugin.",
        ...
        for (PluginWrapper pluginWrapper : getStartedPlugins()) {
            if (pluginWrapper.getPlugin() instanceof PlatformPlugin) {
                ...
                logger.error(
                        ...
                        pluginWrapper.getPlugin().getClass().getName(), PlatformPlugin.class.getName());
            }
        }
        ...
        for (PlatformPlugin mphPlugin : asPlatformPlugins(getStartedPlugins())) {
            for (Class<? extends ConfigurationExtension> group : getConfigGroupExtensions(
                    mphPlugin.getWrapper().getPluginId())) {
    ...
    public Set<Class<? extends ConfigurationExtension>> getConfigGroupExtensions(String pluginId) {
        HashSet<Class<? extends ConfigurationExtension>> set = Sets.newHashSet();
        for (ConfigurationExtension e : getExtensions(ConfigurationExtension.class, pluginId)) {
            set.add(e.getClass());
        }
        logger.info("Detected the following ConfigurationGroup Extensions: " + set);
        return set;
    }
    ...
    private void checkAppStructureLogWarning(File appFolder) {
        ...
        final ArrayList<String> supportedFolders = Lists.newArrayList("data", "data/templates", "plugin.properties",
                "assets", "ldp", "ldp/assets", "ldp/default", "config", "config/repositories", "config/services",
                "config/page-layout", "lib");
        for (File f : FileUtils.listFiles(appFolder, TrueFileFilter.INSTANCE, TrueFileFilter.INSTANCE)) {
