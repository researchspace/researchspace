...
public class SparqlUtil {
    ...
    /**
     * Determines the query type without parsing the query by simply removing the
     * query prolog (prefixes etc). Alternatively one can parse the operation string
     * using
     * {@link QueryParserUtil#parseOperation(org.eclipse.rdf4j.query.QueryLanguage, String, String)}
     * to safely determine the operation type.
     *
     * @param queryString
     * @return
    ...
    public static SparqlOperation getOperationType(QueryTemplate<?> template) {
        if (template instanceof SelectQueryTemplate) {
            ...
            throw new IllegalStateException("template is neither a Select-, Construct-, Ask-, or Update template.");
        }

    }

    // matches case-insensitive and multi-line groups that
    // start with prefix, one to infinite whitespace, followed by any or none
    // character (incl. unicode),
    // followed by a ":", followed zero to infinite whitespace, followed by any
    // character enclosed by "<" and ">"
    // Note: this regex is not strict w.r.t to the standard defined at
    // https://www.w3.org/TR/sparql11-query/#rPN_PREFIX
    // e.g. it would allow a prefix starting with a digit
    // However, as we use it for duplicate checks only (i.e. whether to prepend
    // platform defined prefixes or not) we
    ...
    static Set<String> extractPrefixes(String operationString) {
        ...
        // check if the operation string actually contains "PREFIX" before applying the
        // regex
        // => most queries will not define any prefixes but rely on the namespace
        // definitions
        // of the platform
    ...
    /**
     * Prepends supplied prefixes to the given operationString. Prefixes from the
     * supplied prefixMap will be ignored, if prefixes are already used in the
     * supplied operationString i.e. prefixes defined as part of the query will
     * always overwrite complementary prefixes added by the system.
     *
     * @param operationString - any SPARQL query or update operation string
     * @param prefixMap       map of prefix - namespace values
     * @return
     */
    ...
    public static Set<String> getAllRegisteredWriterMimeTypes() {
        ...
        all.addAll(getRegisteredMimeTypesForQueryType(TupleQueryResultWriterRegistry.getInstance()));
        all.addAll(getRegisteredMimeTypesForQueryType(BooleanQueryResultWriterRegistry.getInstance()));
        return all;
    }

    /**
     * Return registered mime types for a specific SPARQL operation type using the
     * registry of available file formats for this type.
     * 
    ...
    public static TupleQueryResultWriter getTupleQueryResultWriterForAcceptedMediaTypes(OutputStream output,
            List<MediaType> acceptedMediaTypes, TupleQueryResultFormat defaultFormat) {
        ...
        Optional<TupleQueryResultWriterFactory> writerFactory = resultWriterRegistry.get(rdfFormat);
        return writerFactory.get().getWriter(output);
    }

    public static BooleanQueryResultWriter getBooleanQueryResultWriterForAcceptedMediaTypes(OutputStream output,
            List<MediaType> acceptedMediaTypes, BooleanQueryResultFormat defaultFormat) {
        ...
        BooleanQueryResultFormat rdfFormat = (BooleanQueryResultFormat) getFileFormatForAcceptedMediaTypes(
                resultWriterRegistry, acceptedMediaTypes, defaultFormat);
        Optional<BooleanQueryResultWriterFactory> writerFactory = resultWriterRegistry.get(rdfFormat);
        return writerFactory.get().getWriter(output);
    }

    public static RDFWriter getRDFWriterForAcceptedMediaTypes(OutputStream output, List<MediaType> acceptedMediaTypes,
            RDFFormat defaultFormat) {
        RDFWriterRegistry resultWriterRegistry = RDFWriterRegistry.getInstance();
        RDFFormat rdfFormat = getFileFormatForAcceptedMediaTypes(resultWriterRegistry, acceptedMediaTypes,
                defaultFormat);
        Optional<RDFWriterFactory> writerFactory = resultWriterRegistry.get(rdfFormat);
        return writerFactory.get().getWriter(output);
    }

    static <F extends FileFormat> F getFileFormatForAcceptedMediaTypes(FileFormatServiceRegistry<F, ?> registry,
            List<MediaType> acceptedMediaTypes, F defaultFormat) {
        Optional<F> rdfFormatOptional = Optional.empty();
        for (MediaType mediaType : acceptedMediaTypes) {
            rdfFormatOptional = registry.getFileFormatForMIMEType(mediaType.toString());
    ...
    /**
     * Checks whether the query is empty (every line is either empty or is a
     * commentary)
     * 
     * @param queryString
     * @return
    ...
    /**
     * Transforms a given ASK query ASK WHERE { ... } into an equivalent SELECT
