...
public class WikidataSailConnection extends AbstractRESTWrappingSailConnection {
    ...
    public static final IRI SCHEMA_ORG_DESCRIPTION = VF
            .createIRI("http://schema.org/description");
    public static final IRI ORDER_NR = VF
            .createIRI("http://www.metaphacts.com/ontology/orderNr");
    ...
    protected RESTParametersHolder extractInputsAndOutputs(List<StatementPattern> stmtPatterns)
        ...
        stmtPatterns.stream().filter(stmtPattern 
            ...
            .forEach(stmtPattern -> res.getOutputVariables().put(
                    RDFS.LABEL, stmtPattern.getObjectVar().getName()));
        
        stmtPatterns.stream().filter(stmtPattern 
            -> stmtPattern.getPredicateVar().getValue().equals(SCHEMA_ORG_DESCRIPTION)
                && stmtPattern.getSubjectVar().getName().equals(subjVarName)
                && !stmtPattern.getObjectVar().hasValue())
            .forEach(stmtPattern -> res.getOutputVariables().put(
                    SCHEMA_ORG_DESCRIPTION, stmtPattern.getObjectVar().getName()));
        
        stmtPatterns.stream().filter(stmtPattern 
                -> stmtPattern.getPredicateVar().getValue().equals(ORDER_NR)
                    && stmtPattern.getSubjectVar().getName().equals(subjVarName)
                    && !stmtPattern.getObjectVar().hasValue())
                .forEach(stmtPattern -> res.getOutputVariables().put(
                        ORDER_NR, stmtPattern.getObjectVar().getName()));
        ...
        try {
            ...
            for (Object resObj : resList) {
                ...
                if (parametersHolder.getOutputVariables().containsKey(RDFS.LABEL)) {
                    Literal litLabel = VF.createLiteral((String) resMap.get("label"));
                    bs.addBinding(parametersHolder.getOutputVariables().get(RDFS.LABEL),
                            litLabel);
                }
                if (parametersHolder.getOutputVariables().containsKey(SCHEMA_ORG_DESCRIPTION)) {
                    Literal litDesc = VF.createLiteral((String) resMap.get("description"));
                ...
                if (parametersHolder.getOutputVariables().containsKey(ORDER_NR)) {
                    ...
                    bs.addBinding(parametersHolder.getOutputVariables().get(ORDER_NR),
                            litDesc);
                }
