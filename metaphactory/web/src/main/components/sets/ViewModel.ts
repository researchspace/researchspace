...
import {
  SetManagementEvents, SetManagementEventType
...
export class ViewModel {
  ...
  private readonly trigger: (eventType: SetManagementEventType) => void;
  ...
  constructor(options: {
    ...
    trigger: (eventType: SetManagementEventType) => void,
  }) {
  ...
  private startSearching(
    ...
    .observe({
      value: results => this.setState({search: {...state.search, quering: false, results}}),
  ...
  startRenamingSet(targetSet: Rdf.Iri) {
    ...
    this.fetchingSetNameToRename = this.cancellation.derive();
    this.fetchingSetNameToRename.map(getLabel(targetSet)).observe({
      value: oldName => {
        const {sets} = this.getState();
        this.setState({
