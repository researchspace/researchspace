...
public class OpenstreetmapSailConnection extends AbstractRESTWrappingSailConnection {
    ...
    public OpenstreetmapSailConnection(AbstractServiceWrappingSail sailBase) {
        super(sailBase);
    }
    ...
    protected Collection<BindingSet> convertStream2BindingSets(InputStream inputStream,
        ...
        try{
            ...
            JSONArray jsonFile = (JSONArray)Configuration.defaultConfiguration().jsonProvider().parse(mapper.toString());
            ...
            for(Object obj : jsonFile) {
                ...
                // Save display name
                if(parametersHolder.getOutputVariables().containsKey(IRI_DISPLAY_NAME) && !parametersHolder.getOutputVariables().get(IRI_DISPLAY_NAME).isEmpty() ) {
                ...
                // Save geo text
                if (parametersHolder.getOutputVariables().containsKey(IRI_GEO_TEXT) && !parametersHolder.getOutputVariables().get(IRI_GEO_TEXT).isEmpty()) {
                ...
                // Default is 1
                if (parametersHolder.getInputParameters().containsKey(EXTRA_TAGS) && parametersHolder.getInputParameters().get(EXTRA_TAGS).equals("1")) {
                    Map<String, Object>  extratags = (Map<String, Object>) resMap.get(EXTRA_TAGS);

                    // Save wikidata
                    if (parametersHolder.getOutputVariables().containsKey(IRI_WIKIDATA) && !parametersHolder.getOutputVariables().get(IRI_WIKIDATA).isEmpty()) {
                        
                        Literal wikidata = VF.createLiteral((String) (extratags.containsKey(WIKIDATA) ? extratags.get(WIKIDATA) : ""));
    ...
    protected RESTParametersHolder extractInputsAndOutputs(List<StatementPattern> stmtPatterns) throws SailException {
        ...
        if(mandatoryParameters.isEmpty()) {
            throw new SailException("The mandatory parameter was not provided, must be passed via the reserved <"
                    + IRI_Q.stringValue() + "> property."); } // Set parameters to query
        ...
        stmtPatterns.stream()
            .filter(pattern -> pattern.getPredicateVar().getValue().equals(IRI_POLYGON_TEXT))
            .forEach(pattern -> res.getInputParameters().put(POLYGON_TEXT, pattern.getObjectVar().getValue().stringValue()));

        stmtPatterns.stream()
            .filter(pattern -> pattern.getPredicateVar().getValue().equals(IRI_EXTRA_TAGS))
            .forEach(pattern -> res.getInputParameters().put(EXTRA_TAGS, pattern.getObjectVar().getValue().stringValue()));

        // Set output variables
        stmtPatterns.stream()
            .filter(pattern -> pattern.getPredicateVar().getValue().equals(IRI_DISPLAY_NAME))
            .forEach(pattern -> res.getOutputVariables().put(IRI_DISPLAY_NAME, pattern.getObjectVar().getName()));

        stmtPatterns.stream()
            .filter(pattern -> pattern.getPredicateVar().getValue().equals(IRI_GEO_TEXT))
            .forEach(pattern -> res.getOutputVariables().put(IRI_GEO_TEXT, pattern.getObjectVar().getName()));

        stmtPatterns.stream()
            .filter(pattern -> pattern.getPredicateVar().getValue().equals(IRI_WIKIDATA))
