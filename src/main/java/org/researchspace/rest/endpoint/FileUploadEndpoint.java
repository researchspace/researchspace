 ...
 * Simple fileupload
 *
 * Idea: things to abstract in file upload: 1) Uploaded file storage Developer
 * should be able to define virtual storages in server's config and reuse them
 * in components as targets. like 'upload-iiifFolder=file:///var/iiif' in
 * environment.prop and storage 'iiifFolder' in upload component 2) Resource url
 * generation We need a resource to be created for uploaded file. We will use
 * this resource in 2 places: a) in resource description b) in linked data
 * container URI Right now it's a sparql select query for id generation, but
 * this can be simplified to component parameter + random sequence 3) Resource
 * generation and creation of links It's sparql parametrized construct query to
 * create resource
 *
 * TODO: maybe redesign query parameters Right now input parameters are provided
 * for queries - ?__sequence__, ?__newId__, ?__contextUri__ that's a convention
 * easy to miss and meybe should be revised
 *
...
public class FileUploadEndpoint {
    ...
    @Inject
    org.researchspace.config.Configuration systemConfig;
    ...
    public Response createDocument(@FormDataParam("createResourceQuery") String createResourceQuery,
            @FormDataParam("generateIdQuery") String generateIdQuery, @FormDataParam("storage") String storage,
            @FormDataParam("metadataExtractor") String metadataExtractor,
            @FormDataParam("contextUri") String contextUri,
            @FormDataParam("image") FormDataContentDisposition imageDisposition, @FormDataParam("image") InputStream in)
            throws Exception {
        ...
        FileContainer fileContainer = (FileContainer) LDPImplManager.getLDPImplementation(FileContainer.IRI,
                ...
                new MpRepositoryProvider(this.repositoryManager, RepositoryManager.ASSET_REPOSITORY_ID));// ldp.getLDPResource(FileContainer.URI);
        ...
        try {
            ...
            try {
                ...
                throw new Exception("Error creating new id for uploaded file with context " + contextUri + ", sequence "
            ...
            try {
                ...
                throw new Exception("Error creating resource data for uploaded file with context " + contextUri
                        + ", sequence " + sequence + ":" + e.getMessage(), e);
            }
            ...
            try {
                ...
                throw new Exception("Error storing new uploaded file with context " + contextUri + ", sequence "
                        + sequence + ":" + e.getMessage(), e);
            }
            ...
            try {
                if (StringUtils.isNotEmpty(metadataExtractor)) {
                    MetadataExtractor metadataExtractorInstance = (MetadataExtractor) injector
                            .getInstance(Class.forName(metadataExtractor));
                ...
                logger.error("Failed to extract metadata for file with context " + contextUri + ", sequence " + sequence
                        + ":" + e.getMessage());
            ...
            logger.error("Failed to upload file with context " + contextUri + ", sequence " + sequence + ":"
                    + e.getMessage());
            logger.debug("Details:", e);
    ...
    private IRI getGeneratedId(String sequence, String contextUri, String generateIdQuery)
            throws RepositoryException, MalformedQueryException, QueryEvaluationException {
        try (RepositoryConnection con = repositoryManager.getDefault().getConnection()) {
            SparqlOperationBuilder<TupleQuery> operationBuilder = SparqlOperationBuilder.create(generateIdQuery,
                    TupleQuery.class);
            TupleQuery op = operationBuilder.setBinding(SEQUENCE, vf.createLiteral(sequence))
                    .setBinding(CONTEXT_URI, vf.createIRI(contextUri)).build(con);

            // TODO do some checks here and throw/log exceptions
            try (TupleQueryResult result = op.evaluate()) {
                return vf.createIRI(Iterations.asList(result).get(0).getBinding("newId").getValue().toString());
            }
    ...
    private void storeFile(String generatedId, String storage, InputStream in) throws Exception {
        ...
        if (uploadLocation == null) {
            throw new IllegalArgumentException("File storage " + storage + " is not available. upload-" + storage
    ...
    private PointedGraph processCreateResourceQuery(String generatedId, IRI newId, String createResourceQuery,
            String contextUri) throws RepositoryException, MalformedQueryException, QueryEvaluationException {

        try (RepositoryConnection con = repositoryManager.getDefault().getConnection()) {
            SparqlOperationBuilder<GraphQuery> operationBuilder = SparqlOperationBuilder.create(createResourceQuery,
                    GraphQuery.class);
            GraphQueryResult result = operationBuilder.setBinding(SEQUENCE, vf.createLiteral(generatedId))
                    .setBinding(CONTEXT_URI, vf.createIRI(contextUri)).setBinding(NEW_ID, newId).build(con).evaluate();
