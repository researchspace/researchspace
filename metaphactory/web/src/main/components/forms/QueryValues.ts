...
export function fetchInitialValues(
  def: FieldDefinition, subject: Rdf.Iri
  ...
  return queryValues(def.selectPattern, subject).map(values => {
    ...
    // fill loaded values with empty values to always show user
    // at least minOccurs or one field if minOccurs == 0
    const initialValueCount = Math.max(1, def.minOccurs, fieldValues.length);
...
export function validate(
  ...
  const queries = field.constraints.map(constraint =>
    ...
    .flatMap<boolean>(query => SparqlClient.ask(query, '/sparql'))
