...
describe('TreeSelection', () => {
  ...
  const getInsectsWormsAndFish = () => {
    const worms = TreeSelection.selectTerminal(empty, FOREST.getNodePath(FOREST.getFirst('Worms')));
    const insectsAndWorms = TreeSelection.selectTerminal(worms, FOREST.getNodePath(FOREST.getFirst('Insects')));
    const insectsWormsAndFish = TreeSelection.selectTerminal(
      insectsAndWorms,
  ...
  it('keep selection if trying to select child of a leaf', () => {
    const animals = TreeSelection.selectTerminal(empty, FOREST.getNodePath(FOREST.getFirst('Animals')));
    const mammals = TreeSelection.selectTerminal(animals, FOREST.getNodePath(FOREST.getFirst('Mammals')));
    expect(toUnorderedJSON(animals)).to.deep.equal(toUnorderedJSON(mammals));
  });

  it('unselects all nodes with same key', () => {
    const birds = FOREST.nodes
      .get('Birds')
      .reduce((selection, node) => TreeSelection.selectTerminal(selection, FOREST.getNodePath(node)), empty);
