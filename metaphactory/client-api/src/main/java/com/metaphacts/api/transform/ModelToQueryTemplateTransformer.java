...
public class ModelToQueryTemplateTransformer implements ModelToDtoTransformer<QueryTemplate<?>> {
    ...
    public QueryTemplate<?> modelToDto(final Model model) throws InvalidQueryTemplateModelException {
        ...
        try {
            ...
            queryTemplateId = getNotNullSubjectIRI(model, RDF.TYPE, SPIN.TEMPLATE_CLASS);
            queryId = getNotNullObjectIRI(model,queryTemplateId, SPIN.BODY_PROPERTY);
            ...
            for(Value arg : argumentIdentifier){
                ...
                QueryArgument qArgument= new QueryArgument( (IRI) arg,argLabel,argDescription,argPredicate, argValueType);
                
                try{ //required is an optional property
                    Literal required = getNotNullObjectLiteral(model, (Resource) arg, SPL_EXT.REQUIRED_PROPERTY);
                    if(required!=null) qArgument.setRequired(required.booleanValue());
