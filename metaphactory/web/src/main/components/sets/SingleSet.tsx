...
export class SingleSet extends Component<Props, ViewState> {
  ...
  constructor(props: Props, context: any) {
    ...
    this.state = ViewModel.loadState(this.props);
    ...
    this.model = new ViewModel({
      ...
      trigger: eventType => this.trigger(eventType),
    });
  }
  ...
  private trigger(eventType: SetManagementEventType) {
    this.context.GLOBAL_EVENTS.trigger({eventType, source: this.props.id});
  }
  ...
  renderItems() {
    ...
    return (
      <ItemsView key={'default-set-items'}
        ...
        onOrderChanged={order => this.setState({itemsOrdering: order})}
      />
    );
  }

  private onDragStart = () => this.setState({draggingItem: true});
  private onDragEnd = () => this.setState({draggingItem: false});
}
