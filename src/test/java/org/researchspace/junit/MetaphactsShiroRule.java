...
import org.researchspace.security.MetaphactsSecurityTestUtils;
...
public class MetaphactsShiroRule extends ShiroRule {
    ...
    public MetaphactsShiroRule(String iniResourcePath, Supplier<Configuration> configurationSupplier) {
        this((Supplier<Collection<Realm>>) (() -> Collections
                .<Realm>singletonList(MetaphactsSecurityTestUtils.loadRealm(iniResourcePath))), configurationSupplier);
    }
    ...
    private void initializeSecurityManager(SubjectAwareDescriptor subjectAware) {
        ...
        if (realms != null) {
            securityManager = MetaphactsSecurityTestUtils.createInstance(realms.get(), configurationSupplier.get(),
        ...
        if (subjectAware.isMerged()) {
            ...
            if (cfg.length() > 0) {
                securityManager = MetaphactsSecurityTestUtils.createInstance(cfg, configurationSupplier.get(),
                        cacheManagerSupplier.get());
            }
        }
