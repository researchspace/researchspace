...
/**
 * Shouldn't be used directly, see {@link LDPApiInternal} and
 * {@link PermissionsAwareLDPApi}.
 *
...
class LDPApi {
    ...
    protected LDPResource createLDPResource(Optional<String> slug, RDFStream stream, IRI targetContainer,
            String instanceBase, Consumer<LDPContainer> permissionCheck) {
        LDPResource target = LDPImplManager.getLDPImplementation(targetContainer,
                getLDPTypesFromRepository(targetContainer), this.repositoryProvider);
    ...
    private Model removeContainerItemMetadata(Model model, List<IRI> rootIRIs) {
        ...
        for (Statement triple : model) {
            boolean isMetadataPO = triple.getPredicate().equals(RDF.TYPE)
                    && (triple.getObject().equals(PROV.Entity) || triple.getObject().equals(LDP.Resource))
            ...
            boolean isMetadataS = rootIRIs.contains(triple.getSubject())
    ...
    /**
     * Changes root of container items with string replacement for any related IRIs
     * like root/body
     */
    ...
    protected LDPResource copyLDPResource(Optional<String> slug, IRI uri, Optional<IRI> targetContainer,
            String instanceBase, BiConsumer<IRI, LDPResource> resourcePermissionCheck,
        ...
        LDPResource target = LDPImplManager.getLDPImplementation(parentContainer,
                getLDPTypesFromRepository(parentContainer), this.repositoryProvider);
        if (!LDPContainer.class.isAssignableFrom(target.getClass())) {
    ...
    /**
     * Export recursively LDP resources with everything it ldp:contains and hint
     * about container types where it can be stored
     */
    ...
    private void getImportTopologicalSortDFS(Map<IRI, Set<IRI>> adjacencyList, IRI root, List<IRI> result,
            Set<IRI> used) {
        used.add(root);
    ...
    private LDPContainer getImportContainerToAddInto(IRI parentIRI) {
        LDPResource parentResource = LDPImplManager.getLDPImplementation(parentIRI,
                getLDPTypesFromRepository(parentIRI), this.repositoryProvider);
    ...
    private List<IRI> addImportGraph(List<IRI> roots, Model graph, String instanceBase,
        ...
        // in order of topsort add triples from child context into container of A,
        // rename if needed
    ...
    /**
     * Import exported LDP resource, recursively adding new (possibly renamed) items
     * into containers Steps: 1) determine root of import data by hint bnode
     * ldp:contains root 2) determine set of containers where data can be import
     * into by hint bnode a list_of_types and SPARQL query 3) remove provenance
     * metadata (added by LDPContainer.add) and hint from data 4) determine set of
     * unknown objects 5) if import cannot be done then return null, request will be
     * put into delayed cache for clarifying params 6) run topological sort over
     * ldp:contains DAG for determining order of inserts (higher = earlier) 7) by
     * order of topsort: prepare single entity graph by context create LDP resource
     * (possibly renamed) rename entity data IRIs according to created resource
     * (currently by prefix substitute) add resource and save it created IRI for
     * further inserts
     * 
     * @param resource           Model of data to import
     * @param possibleContainers Will store containers from DB where data can be
     *                           imported into
     * @param containerIRI       If there are more than one possible container, user
     *                           can choose one of
     ...
     * @param instanceBase
     * @return Imported resource or null if |possibleContainers| != 1 and
     *         containerIRI is not set or if |unknownObjects| > 0 and force=false
     */
    protected List<LDPResource> importLDPResource(Model resource, Set<IRI> possibleContainers,
        ...
        // if import request can be done immediately, do it, else put it in delayed
        // cache
        boolean success = (unknownObjects.isEmpty() || force)
                && (possibleContainers.size() == 1 || containerIRI.isPresent());
        if (!success) {
            return null;
        }
    ...
    protected LDPResource updateLDPResource(RDFStream stream, IRI resourceToUpdate,
    ...
    protected LDPResource updatePointedGraph(PointedGraph pointedGraph, LDPResource toUpdate, IRI resourceToUpdate,
            BiConsumer<IRI, LDPResource> permissionCheck) {
    ...
    /**
     * 1) If slug is valid IRI: if it is already in use as a resource IRI throw
     * IllegalArgumentException otherwise use it as a new resource IRI. 2) If slug
     * is not an IRI, then use it as a local name of new IRI. 3) If slag contains
     * illegal characters the new constructed IRI is not a valid IRI then throw
    ...
    protected IRI createResourceIRI(Optional<String> slug, IRI targetContainer, String newInstanceBase) {
        ...
        if (isValidIRIReference(_slug)) {
            if (resourceAlreadyExistsInContainer(vf.createIRI(_slug))) {
                throw new IllegalArgumentException(
                        "Resource with the given IRI already exists in some LDP container: " + _slug);
            } else {
        ...
        IRI iri = vf.createIRI(host + targetContainer.getLocalName() + "/" + localName); // TODO something better than
        ...
        if (resourceAlreadyExistsInContainer(iri))
            return createResourceIRI(Optional.ofNullable(RandomStringUtils.randomAlphabetic(40).replaceAll("\\s", "_")),
                    targetContainer, newInstanceBase);
        return iri;
    }
    ...
    private PointedGraph deserialize(RDFStream stream, IRI newResourceUri)
            throws RDFParseException, UnsupportedRDFormatException {
        ...
        if (newResourceUri.stringValue().contains("#")) {
            if (logger.isTraceEnabled())
                logger.trace("Suggest IRI for new resource contains hash. Using a dummy IRI for parsing.");
            _newResourceUri = vf.createIRI(
                    "http://www.baseURI.org/" + RandomStringUtils.randomAlphabetic(40).replaceAll("\\s", "_"));
        }
        ...
        // skip this check for sets and semantic narrative
        // clipboard is only visible by all, but editable by logged in user
        // clipboard can be shared
        // semantic narrative are visible by all if saved by anonymous
