...
export class TemplateItem extends Component<TemplateItemProps, State> {
  ...
  render() {
    ...
    return typeof root === 'string' ? D.span({}, root) :
     (root ? cloneElement(root, {
      ...this.props.componentProps,
      className: classNames(has(this.props.componentProps, 'className') ? this.props.componentProps.className : '', root.props.className),
  ...
  private compileTemplate(props) {
    const {templateDataContext = new CapturedContext()} = this.context;

    const capturer = templateDataContext.capture();
