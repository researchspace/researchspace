...
import org.researchspace.config.groups.ConfigurationGroupBase;
import org.researchspace.config.groups.DataQualityConfiguration;
import org.researchspace.config.groups.EnvironmentConfiguration;
import org.researchspace.config.groups.GlobalConfiguration;
...
public class Configuration {
    ...
    /**
     * The key of the system property to look-up {@link #getRuntimeDirectory()}
     * property.
     */
    // TODO: should be private but still used in tests to set temporary runtime
    // directory
    public static final String SYSTEM_PROPERTY_RUNTIME_DIRECTORY = "runtimeDirectory";

    /**
     * The key of the system property to look-up {@link #getConfigBasePath()}
     * property.
     */
    private static final String SYSTEM_PROPERTY_RUNTIME_CONFIG_BASE = "org.researchspace.config.baselocation";
    ...
    /**
     * A registry of configuration groups administered by the config. The registry
     * is used for automatic lookup via the REST API.
     */
    private final Map<String, ConfigurationGroup> registry;

    @Inject
    public Configuration(GlobalConfiguration globalConfig, UIConfiguration uiConfig,
            EnvironmentConfiguration environmentConfig, DataQualityConfiguration dataQualityConfig) {
     ...
     * 
     * <pre>
     * org.researchspace.config.baselocation
     * </pre>
     * 
     * (see {@link #DEFAULT_CONFIG_BASE_LOCATION}).
     *
    ...
    public static String getConfigBasePath() {
        ...
        if (configBaseLocation == null) {
            ...
            return configBaseLocation.endsWith("/") ? configBaseLocation : configBaseLocation + "/";
        }
    }
     ...
     * empty. The group is only registered if there exists no configuration group
     * with the given ID.
     *
     * @param configurationGroup the group to register
     * @return true if the group was registered, false otherwise
     */
    final public boolean registerCustomConfigurationGroup(final ConfigurationGroup configurationGroup) {
        ...
        if (StringUtils.isEmpty(customGroupId)) {
            throw new IllegalArgumentException("Cannot register configuration group: ID must not be null or empty.");
        }
     ...
     * specified clazz. Returns null of the config group does not exist or the cast
     * fails (in the latter case, a WARNING will be logged out in addition).
     *
     * This is the proper mechanism to access configuration groups that are hooked
     * in via plugins, see the sample plugin for example code.
     *
     * @param configGroupId ID of the configuration group
     * @param clazz         expected clazz of the configuration group
     *
     * @return the configuration group with the given ID or null, if not defined
     */
    final public <T extends ConfigurationGroup> T getCustomConfigurationGroup(final String configGroupId,
            final Class<T> clazz) {
        try {
            return clazz.cast(registry.get(configGroupId));
        } catch (ClassCastException e) {
            logger.warn("Error casting to configuration group class: " + e.getMessage());
        }
     ...
     *
     * <p>
     * <strong>Please note security checks need to done outside of this method i.e.
     * the map being returned contains all configuration parameters regardless of
     * whether the user has the respective permissions!</strong>
     * </p>
     *
     * @param configGroup config group for which the lookup is performed
     * @return a map from identifiers to configuration parameter value information
     *         objects
     */
    public Map<String, ConfigParameterValueInfo> listParamsInGroups(final String configGroup)
     ...
     * @throws UnknownConfigurationException If the config group or parameter in the
     *                                       group does not exit or there are any
     *                                       unexpected expections during the
     *                                       lookup.
     */
    public ConfigParameterValueInfo lookupProperty(final String configGroup, final String configIdInGroup)
            throws UnknownConfigurationException {

        // assert parameters are set correctly
        if (StringUtils.isEmpty(configGroup) || StringUtils.isEmpty(configIdInGroup)) {
            throw new UnknownConfigurationException();
        }
     ...
     * @throws UnknownConfigurationException If the config group or parameter in the
     *                                       group does not exit or there are any
     *                                       unexpected exceptions while writing the
     *                                       property.
     * @throws ConfigurationException
     */
    public void setProperty(String configGroup, String configIdInGroup, List<String> configValues, String targetAppId)
        ...
        ConfigurationGroup group = registry.get(configGroup);
        if (group == null) {
            throw new UnknownConfigurationException("Configuration group: \"" + configGroup + "\" is unknown.");
        }

        // prevent writing if property is shadowed by runtime configuration
        String systemPropertyName = ConfigurationUtil.configParamToSystemParam(configGroup, configIdInGroup);
        String systemProperty = System.getProperty(systemPropertyName);

        if (StringUtils.isNotEmpty(systemProperty)) { // shadowed by -Dconfig....
            throw new UnauthorizedException("Configuration element is shadowed by system parameter. "
        ...
        if (group instanceof EnvironmentConfiguration) {
            throw new UnauthorizedException("Not allowed to change environment configuration at runtime.");
        }

        // invoke method and return result
        try {
            if (group instanceof ConfigurationGroupBase) {
                ((ConfigurationGroupBase) group).setParameter(configIdInGroup, configValues, targetAppId);
            } else {
                throw new UnknownConfigurationException();
            }
        } catch (Exception e) {
            ...
            logger.warn("Exception during setting values [" + configValues + "]\" for configuration property \""
                    + configIdInGroup + "\"", e);
            throw new UnknownConfigurationException();
        }
    }
