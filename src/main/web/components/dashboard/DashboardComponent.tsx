...
export class DashboardComponent extends Component<Props, State> {
  ...
  private onAddNewItem = (item: Item = this.frameLabel()) => { 
    ...
    const itemIsAlreadyOpen = this.state.items.filter((i) => (i.resourceIri === item.resourceIri && i.viewId === item.viewId ) && !((item.viewId === "resource-editor") && (item.resourceIri === undefined)));
    //&& ((item.viewId === "resource-editor") && (item.resourceIri === undefined)
