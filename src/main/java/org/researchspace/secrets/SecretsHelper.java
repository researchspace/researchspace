...
public class SecretsHelper {
     ...
     * resolver is provided and the key has placeholder markers. Otherwise the
     * original value or <code>null</code> is returned. Any exceptions in a
     * {@link SecretResolver} are caught and ignored, instead the fallback value is
     * returned.
     * 
     ...
     * @param value          key to resolve. The format of the lookup string is
     *                       {@code ${key:fallback}) where {@code key} specifies the
     *                       key to look up using a {@link SecretResolver} and
     *                       {@code fallback} the fallback value which is returned
     *                       if the lookup did not return a value. @return if value
     *                       contains a lookup key, the resolved secret is returned.
     *                       If the secret could not be resolved either the fallback
     *                       value (if specified) or <code>null</code> is returned.
     *                       if value does not contain a lookup key, it is returned
     *                       directly
    ...
    /**
     * Resolve secret (if applicable). Secrets are only resolved if a resolver is
     * provided and the key has placeholder markers. Any exceptions in a
     * {@link SecretResolver} are caught and ignored, instead an empty result is
     * returned.
     * 
     * @param secretResolver secrets resolver or <code>null</code> if not available.
     *                       In this case the provided key is passed unchanged
     ...
     *                       key to look up using a {@link SecretResolver} and
     *                       {@code fallback} the fallback value which is returned
     *                       if the lookup did not return a value. @return resolved
     *                       secret. If the secret could not be resolved an empty
     *                       {@link Optional} is returned
     * @see #isResolvableSecret(String)
     */
    public static Optional<String> resolveSecretAsString(@Nullable SecretResolver secretResolver,
    ...
    /**
     * Resolve secret (if applicable). Secrets are only resolved if both a resolver
     * and lookup are provided. Any exceptions in a {@link SecretResolver} are
     * caught and ignored, instead an empty result is returned.
     * 
     * @param secretResolver secrets resolver or <code>null</code> if not available.
     *                       In this case the provided key is passed unchanged
     * @param lookup         secret lookup to resolve
     * @return resolved secret. If the secret could not be resolved the
     *         {@link SecretLookup}'s fallback value is returned if provided,
     *         otherwise an empty {@link Optional}.
     */
    public static Optional<String> resolveSecretAsString(@Nullable SecretResolver secretResolver,
            Optional<SecretLookup> lookup) {
        Secret secret = null;
        try {
     ...
     *         otherwise an empty {@link Optional}.
     */
    public static Optional<Secret> resolveSecret(@Nullable SecretResolver secretResolver,
            Optional<SecretLookup> lookup) {
        if (!lookup.isPresent()) {
            return Optional.empty();
        }
