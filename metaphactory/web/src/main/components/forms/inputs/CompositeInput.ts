...
import { DOM as D, createElement, Props } from 'react';
...
import {
  fieldInitialState, generateSubjectByTemplate, loadDefaultsAndValueSets, tryBeginValidation,
} from '../FormModel';
...
export class CompositeInput extends SingleValueInput<ComponentProps, {}> {
  ...
  private loadComposite(props: ComponentProps) {
    ...
    this.compositeOperations.map(
      ...
      loadDefaultsAndValueSets(rawComposite, inputs)
  ...
  finalize(owner: EmptyValue | CompositeValue, value: FieldValue): CompositeValue {
    const finalizedComposite = this.finalizeSubject(owner, value);

    const fields = finalizedComposite.fields.map((state, fieldId) => {
      const input = this.inputs[fieldId];
      const values = input
