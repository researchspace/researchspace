...
define("tinymce/tableplugin/TableGrid", [
	...
	return function(editor, table) {
		...
		function buildGrid() {
			...
			each(['thead', 'tbody', 'tbart'], function(part) {
		...
		function cleanup() {
			...
			// Empty header/body/barter
			each(dom.select('thead,tbody,tbart', table), function(part) {
			...
			function eventHandler(e) {
				...
				function handle(upBool, sourceNode) {
					...
					if (siblingRow) {
						...
						if (parentNodeName === 'tbody' || parentNodeName === (upBool ? 'tbart' : 'thead')) {
				...
				function getFirstHeadOrFoot(upBool, parent) {
					var tagName = upBool ? 'thead' : 'tbart';
