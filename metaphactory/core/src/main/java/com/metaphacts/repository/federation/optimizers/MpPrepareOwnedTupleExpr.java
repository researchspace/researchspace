...
package com.metaphacts.repository.federation.optimizers;
...
import com.metaphacts.sparql.VariableRetrievingVisitor;
...
public class MpPrepareOwnedTupleExpr extends AbstractQueryModelVisitor<RepositoryException>
    ...
    private void meetOwnedTupleExpr(MpOwnedTupleExpr node) throws RepositoryException {
        ...
        try {
            ...
            pattern = "SELECT " 
                            + projectionBuilder.toString() 
                            + " WHERE { "  
                            + serializer.serialize(stq) 
                            + "}";
        } catch (Exception e) {
