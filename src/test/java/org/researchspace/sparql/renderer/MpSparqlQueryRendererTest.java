...
public class MpSparqlQueryRendererTest {
    ...
    private String loadQuery(String queryId) throws Exception {
        return SparqlTestUtils.loadClasspathResourceAsUtf8String(MpSparqlQueryRendererTest.class, queryId + ".sq");
    }
    ...
    private void testSingleQuery(String id, String strQuery) {
        ...
        ParsedOperation query = QueryParserUtil.parseOperation(QueryLanguage.SPARQL, strQuery, null);
        String renderedQuery = "";
        try {
            renderedQuery = mpQueryRenderer.render(query);
        } catch (Exception e) {
            throw new AssertionError("Could not render a parsed query " + id + ": " + e.getMessage(), e);
        }
        try {
            ParsedOperation restoredQuery = QueryParserUtil.parseOperation(QueryLanguage.SPARQL, renderedQuery, null);
            query = getCanonicalForm(query);
            restoredQuery = getCanonicalForm(restoredQuery);
            Assert.assertEquals(
                    "Error in the query " + id + " defined as:\n" + strQuery + "\nThe rendered query was:\n"
                            + renderedQuery,
                    replaceAnonymous(query.toString()), replaceAnonymous(restoredQuery.toString()));
        } catch (Exception e) {
            throw new AssertionError(
                    "Could not parse the rendered query: [" + renderedQuery + "]. Reason: " + e.getMessage(), e);
        }
    }
