...
/**
 * Convenience to handle handlebars templates. In particular, generating and
 * extracting includes for the template mechanism.
 * 
...
public class TemplateUtil {
    ...
    /**
     * Converts a set of {@link Resource}s to template identifiers by prepending the
     * "Template:" prefix <code>["foaf:Person","foaf:Agent"]</code> Will be
     * converted into: <code>["Template:foaf:Person","Template:foaf:Agent"]</code>
     * 
     ...
     * will be converted into Template:foaf:Agent
     * 
     * @param r
     * @return
     */
    ...
    /**
     * Calls {@link #extractIncludeStrings(String)} and converts IRI strings to
     * {@link IRI} including resolving prefixed IRIs to full IRIs using the supplied
     * {@link NamespaceRegistry}.
     * 
     * @param rawTemplateText Any handlebars.java string.
     * @param ns              {@link NamespaceRegistry} to resolve prefixed include
     *                        resources to full {@link IRI}s
    ...
    /**
     * <p>
     * Extracts all includes from a raw template text.
     * </p>
    ...
    /**
     * Tries to resolve a template from the specified location through the supplied
     * template loader. Catches {@link IOException}s silently (meaning not template
     * exists) and wraps the resulting {@link TemplateSource} into an
     * {@link Optional}.
     * 
     * @param loader
     * @param location
    ...
    /**
     * Returns the identifier of the first template that can is source-able (exists)
     * from the supplied, ordered set of template location strings. Returns an empty
     * optional if none of the templates can be loaded.
     * 
     * @param loader
     * @param orderedSetOfLocations
     * @return
     */
    public static Optional<String> findFirstExistingTemplate(TemplateLoader loader,
            LinkedHashSet<String> orderedSetOfLocations) {
        return orderedSetOfLocations.stream().filter(l -> TemplateUtil.getTemplateSource(loader, l).isPresent())
                .findFirst();
    }
     ...
     * @return
     */
    public static LinkedHashSet<String> getRdfTemplateIncludeIdentifiers(Value pageId, TemplateContext tc,
     ...
     */
    public static Optional<String> compileAndReturnFirstExistingTemplate(TemplateContext tc,
            LinkedHashSet<String> appplicableTemplates, Handlebars handlebars)
            throws MissingHelperException, IllegalArgumentException {
        try {
            // returns the first template that can be loaded (exists) from the ordered set
            ...
            for (String location : appplicableTemplates) {
                ...
                if (source.isPresent()) {
                    ...
                    if (logger.isTraceEnabled()) {
                        logger.trace("Found the following handlebar tags {} in template: {}",
                                template.collect(TagType.values()), location);
                    }
