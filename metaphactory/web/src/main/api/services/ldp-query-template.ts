...
import { LdpService, LdpServiceContext, RootContainer } from './ldp';
import { includes } from 'lodash';

const { VocabPlatform, xsd, ldp, rdf, rdfs, spl, spin, dct } = vocabularies;
...
export class QueryTemplateServiceClass extends LdpService {
  ...
  public addItem(template: Template, queryIri: string, namespace: string): Kefir.Property<{}> {
    ...
    const addTemplateGraph = () => this.addResource(graph, maybe.Just(template.identifier));

    return RootContainer(this.context)
      ...
      .flatMapErrors<Rdf.Iri>(
        ...
        e => this.createContainer().flatMap(addTemplateGraph)
      ).toProperty();
  }
  ...
  private createContainer(): Kefir.Property<Rdf.Iri> {
    return RootContainer(this.context).addResource(
      Rdf.graph([
        Rdf.triple(
          Rdf.iri(''),
          rdf.type,
        ...
        Rdf.triple(
          Rdf.iri(''),
          rdfs.label,
          Rdf.literal('SPIN Query Templates')
        ),
