...
import org.researchspace.data.rdf.container.DefaultLDPContainer;
...
/**
 * This container holds overlayed images. From two source images it gets them
 * from image server, blends to create 3rd image, saves it into image server and
 * provides appropriate metadata. this container expects some initial
 * information on blended image and provides some additional metadata.
 *
 * Digital object 1 -> L21 -> D3 ; Digital Object 2 -> L21 -> D3 ; D3 -> L22 ->
 * Digital Object 3
 *
 * could use to represent blending algorithm: D3 - rso:blendingType ->
 * :scos-thesauri
 *
 * we need to preserve order and specify opacity, thus:
 *
 * D3 - L13_used_parameters -> arg1 arg1 - rdf:type - D1_Digital_Object arg1 -
 * rso:image Digital Object 2 arg1 - rso:order 1 arg1 - rso:opacity 0.4
 *
...
public class OverlayImageContainer extends DefaultLDPContainer {
    ...
    private java.net.URI getIIIFFolder() throws RepositoryException {
        ...
        try {
            if (Strings.isNullOrEmpty(iiifFolder))
                throw new Exception(
                        "Need to set " + IIIF_PATH_PARAM + " parameter to point to IIIF Image Server storage");
    ...
    public OverlayImageProcessor getOverlayImageProcessor() throws RepositoryException {
        if (overlayImageProcessor == null)
            overlayImageProcessor = new OverlayImageProcessor(getIIIFFolder(), ns.getUserIRI());
        return overlayImageProcessor;
    }

    @Override
    public IRI getContextIRI() {
        // for image regions the contextURI i.e. URI of named graph must be equivalent
        // to identifier of the region
        // that is if we use image regions as premises we point to the entire named
        // graph
