...
/**
 * Copies {@link FederationJoinOptimizer}, but assumes an explicit federation as
 * opposed to a transparent one.
 * 
...
public class MpFederationJoinOptimizer extends AbstractQueryModelVisitor<RepositoryException>
    ...
    /*
     * private static Map<Resource, List<RepositoryConnection>>
     * createContextToMemberMap( Collection<? extends RepositoryConnection> members)
     * throws RepositoryException { Map<Resource, List<RepositoryConnection>>
     * contextToMemberMap = new HashMap<Resource, List<RepositoryConnection>>(
     * members.size() + 1); for (RepositoryConnection member : members) {
     * RepositoryResult<Resource> res = member.getContextIDs(); try { while
     * (res.hasNext()) { Resource ctx = res.next(); List<RepositoryConnection>
     * contextMembers = contextToMemberMap.get(ctx); if (contextMembers == null) {
     * contextMembers = new ArrayList<RepositoryConnection>();
     * contextToMemberMap.put(ctx, contextMembers); } contextMembers.add(member); }
     * } finally { res.close(); } } return contextToMemberMap; }
     */
    ...
    protected boolean shouldMergeForLocalJoin(TupleExpr arg, List<Owned<NaryJoin>> ows, RepositoryConnection owner) {
        // The local join is possible if the previous join operand has the same target
        // as this one
    ...
    protected void meetMultiJoin(NaryJoin node) throws RepositoryException {
        ...
        for (TupleExpr arg : node.getArgs()) {
            ...
            // Check whether two consequent operands are executed at the same target
            // ("owner")
            // If yes, it does not make sense to send them separately and then join at the
            // Ephedra side,
    ...
    protected class OwnerScanner extends AbstractQueryModelVisitor<RepositoryException> {
        ...
        public void meet(Service node) throws RepositoryException {
            if (node.getServiceRef().getValue() instanceof IRI) {
                Repository mappedRepository = MpFederationJoinOptimizer.this.serviceMappings
                        .get(node.getServiceRef().getValue());
        ...
        public void meet(StatementPattern node) throws RepositoryException {
             ...
             * (IRI)node.getPredicateVar().getValue(); Value obj =
             * node.getObjectVar().getValue(); Resource[] ctx =
             * getContexts(node.getContextVar()); RepositoryConnection member =
             * getSingleOwner(subj, pred, obj, ctx); if (member == null) { multipleOwners();
             * } else { usedBy(member); }
             */
        }
        ...
        /*
         * private Resource[] getContexts(Var var) { return (var == null ||
         * !var.hasValue()) ? new Resource[0] : new Resource[] {
         * (Resource)var.getValue() }; }
         */

        /*
         * private RepositoryConnection getSingleOwner(Resource subj, IRI pred, Value
         * obj, Resource[] ctx) throws RepositoryException { RepositoryConnection result
         * = null;
         * 
         * // Avoid querying repositories if given a set of explicit contexts only
         * belonging to one federation member if (contextToMemberMap == null) {
         * contextToMemberMap = createContextToMemberMap(members); }
         * Set<RepositoryConnection> results = new HashSet<RepositoryConnection>();
         * Collection<Resource> explicitContexts; if (ctx.length > 0) { explicitContexts
         * = Arrays.asList(ctx); } else if (dataset != null) { // all graphs
         ...
         * = Collections.<Resource> emptyList(); } for (Resource context :
         * explicitContexts) { List<RepositoryConnection> contextRepos =
         * contextToMemberMap.get(context); if (contextRepos != null) {
         * results.addAll(contextRepos); } } if (results.size() == 1) { result =
         * results.iterator().next(); } else { // fallback to using hasStatement() //
         * but hopefully we narrowed it down to results for (RepositoryConnection member
         * : results) { RepositoryBloomFilter bloomFilter = MoreObjects.firstNonNull(
         * bloomFilters.apply(member.getRepository()),
         * AccurateRepositoryBloomFilter.INCLUDE_INFERRED_INSTANCE); if
         * (bloomFilter.mayHaveStatement(member, subj, pred, obj, ctx)) { if (result ==
         * null) { result = member; } else if (result != member) { result = null; //
         * NOPMD break; } } } } return result; }
         */
    ...
    /**
     * If two basic graph patterns have the same subject and can be run on the same
     * member, we can change the order.
     */
    ...
    private void addDistinctOwnersNonLocal(LeftJoin node, RepositoryConnection leftOwner,
        ...
        if (rightOwner != null) {
            node.getRightArg().replaceWith(new MpOwnedTupleExpr(rightOwner, node.getRightArg().clone()));
        }
    }
