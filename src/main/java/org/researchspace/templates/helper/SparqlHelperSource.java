...
public class SparqlHelperSource {
    ...
    protected String renderWithParameters(ParsedQuery parsedQuery, Map<String, Value> parameters) throws Exception {
    ...
    /**
     * Renders a query template from the query catalog, setting the parameters if
     * necessary.
     *
     * <p>
     * Example:
    ...
    public String getQueryString(String param0, Options options) throws Exception {
        ...
        QueryTemplate<?> queryTemplate = queryTemplateCache.getQueryTemplate(VF.createIRI(queryTemplateId));
                ...
                if (arg.getValueType() != null) {
                    if (arg.getValueType().equals(XMLSchema.ANYURI) || arg.getValueType().equals(RDFS.RESOURCE)) {
                        ...
                        if (val instanceof Literal) {
                            ...
                            throw new IllegalArgumentException(
                                    "Incompatible parameter type: a resource " + val.stringValue() + " passed where <"
                                            + arg.getValueType().stringValue() + "> expected.");
                        }
                    }
                } else {
