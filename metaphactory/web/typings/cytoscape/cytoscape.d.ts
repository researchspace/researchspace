...
declare module Cy {
  ...
  */
  ...
  type Stylesheet = CssStylesheet | StyleStylesheet
  type Stylesheets = Array<Stylesheet>;
  ...
  export module Css {
     ...
     */
    type Percent = number;
    type Position = 'parent' | 'origin';
    type NodeSize = number | 'auto' | 'label';
    type BackgroundWidth = number | 'auto';
    type BgRepeat = 'repeat' | 'repeat-x' | 'repeat-y' | 'no-repeat';
    type BgFit = 'none' | 'contain' | 'cover';
    type BgClip = 'none' | 'node';
    type Colour = string;
    type LineStyle = 'solid' | 'dotted' | 'dashed';
    type BorderStyle = 'solid' | 'dotted' | 'dashed' | 'double';
    type CurveStyle = 'bezier' | 'unbundled-bezier' | 'haystack' | 'segments';
    type FontVariant = 'small-caps' | 'normal';
    type FontStyle = 'italic' | 'normal' | 'oblique';
    type FontWeight = 'normal' | 'bold' | 'bolder' | 'lighter' | '100' | '200' | '300' |
      '400' | '500' | '600' | '800' | '900' | number;
    type TextDecaration = 'none' | 'underline' | 'overline' | 'line-through';
    type TextTransform = 'none' | 'uppercase' | 'lowercase';
    type TextWray = 'none' | 'wrap';
    type TextBackgroundShape = 'rectangle' | 'roundrectangle';
    type TextRotation = number | 'none' | 'autorotate';
...
// }
    ...
    type Shape = 'rectangle' | 'roundrectangle' | 'ellipse' |
      'triangle' | 'pentagon' | 'hexagon' | 'heptagon' | 'octagon' | 'star';

    type EdgeArrowShape = 'tee' | 'triangle' | 'triangle-tee' | 'triangle-backcurve' |
      'square' | 'circle' | 'diamond' | 'none';

    type ValueFunction<T> = (elem: CollectionFirst) => T;
    type Value<T> = T | ValueFunction<T>;
    ...
    export interface NodeCss extends BaseCss {
    ...
    export interface EdgeCss extends BaseCss {
      'control-point-distances'?: string;
      'curve-style'?: CurveStyle;
      'text-rotation'?: Value<TextRotation>;
      'line-color'?: Colour;
      'line-style'?: LineStyle;
      'target-arrow-shape'?: EdgeArrowShape;
      'target-arrow-color'?: Value<Colour>;
    }

    export interface ElementCss extends NodeCss, CompoundNodeCss, EdgeCss { }
  }
  ...
  interface Instance extends InstanceEvent, InstanceViewPort, InstanceAnimation, InstanceLayout, InstanceExport {
    /**
  ...
  interface InstanceEvent {
     ...
     */
    ready(fn: (event: EventObject) => void): void;
  }
  ...
  type InstanceStyle = string | InstanceJsonStyle | Css.ElementCss;
  type InstanceJsonStyle = Array<{ selector: Selector, style: Css.ElementCss }>
  ...
  type IdealEdgeLengthCose = () => number;
  type IdealEdgeLengthCoseBilkent = number;

  type IdealEdgeLength = IdealEdgeLengthCose | IdealEdgeLengthCoseBilkent;
  ...
  interface BreadthfirstLayoutOptions extends LayoutOptions {
    roots?: string[];
  }
