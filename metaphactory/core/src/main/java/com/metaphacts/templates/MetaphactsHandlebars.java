...
public class MetaphactsHandlebars extends Handlebars{
    ...
    private void initialize(TemplateLoader templateLoader, HandlebarsHelperRegistry helperRegistry) {
        ...
        if(templateLoader!=null){
            logger.info("Initialized metaphactory handlebars engine with template loader:  "+templateLoader.getClass());
        }else{
            logger.error("Initialized metaphactory handlebars engine with no template loader. Default (classpath) template loader will be used.");
        }
