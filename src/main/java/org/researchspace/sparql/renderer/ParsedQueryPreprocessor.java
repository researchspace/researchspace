...
/**
 * A query visitor that parses the incoming query or TupleExpr and collects
 * meta-level information that is later used by the
 * {@link MpSparqlQueryRenderer} (e.g., information about the included subquery,
 * all group, order, and slice operations etc.).
 * 
...
public class ParsedQueryPreprocessor extends AbstractQueryModelVisitor<RuntimeException> {
    ...
    /**
     * Processes the incoming parsed query collecting the information required for
     * rendering.
     * 
     * @param query standard {@link ParsedTupleQuery}
     * @return {@link SerializableParsedTupleQuery} containing the original query
    ...
    public SerializableParsedConstructQuery transformToSerialize(ParsedGraphQuery query) {
        ...
        if (this.graphQueryProjection == null) {
            ...
            this.graphQueryProjection
                    .setProjections(Lists.newArrayList(currentQueryProfile.projection.getProjectionElemList()));
    ...
    /**
     * Processes the incoming parsed ASK query collecting the information required
     * for rendering.
     * 
     * @param query standard {@link ParsedBooleanQuery}
     * @return {@link SerializableParsedBooleanQuery} containing the original query
     *         and the required additional information.
     */
    ...
    /**
     * Processes the incoming parsed {@link TupleExpr} collecting the information
    ...
    /**
     * If we have a VALUES clause inside the WHERE clause, we should not render it
     * twice.
     */
    protected void cleanBindingSetAssignments(SerializableParsedTupleQuery query) {
        if (query.bindings != null && query.whereClause != null && isAncestor(query.whereClause, query.bindings)) {
            query.bindings = null;
        }
    }
    ...
    public void meet(MultiProjection node) throws RuntimeException {
        ...
        node.getProjections().stream().forEach(projList -> projList.getElements().stream()
                .forEach(elem -> fakeProjection.getProjectionElemList().addElement(elem)));
