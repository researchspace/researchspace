...
CONSTRUCT {
  ...
  ?config <http://www.researchspace.org/pattern/system/resource_configuration/has_view> ?view .
  ?view <http://www.researchspace.org/pattern/system/resource_configuration/view_has_label> ?viewLabel .
      ...
      FieldService.getGeneratedFieldDefinitions(allRelatedKps).onValue(
        allKps => {
          ...
          const allConfigs = allConfigIris.map(configIri => {
            ...
            const facetKps =
              Rdf.getValuesFromPropertyPath<Rdf.Iri>([Rdf.iri('http://www.researchspace.org/pattern/system/resource_configuration/has_facet_kp')], pg);

            const viewIris = Rdf.getValuesFromPropertyPath<Rdf.Iri>([Rdf.iri('http://www.researchspace.org/pattern/system/resource_configuration/has_view')], pg);
            ...
            let viewes =
              viewIris.map(
                viewIri => {
                  ...
                  const label = Rdf.getValueFromPropertyPath<Rdf.Literal>([Rdf.iri('http://www.researchspace.org/pattern/system/resource_configuration/view_has_label')], viewPg).map(l => l.value).get();
                  const order = Rdf.getValueFromPropertyPath<Rdf.Literal>([Rdf.iri('http://www.researchspace.org/pattern/system/resource_configuration/view_has_order')], viewPg).map(l => parseInt(l.value)).get();
                  const templateIri = Rdf.getValueFromPropertyPath<Rdf.Literal>([Rdf.iri('http://www.researchspace.org/pattern/system/resource_configuration/view_has_template')], viewPg).map(l => l.value).get();
            ...
            const navigationEntries =
              navigationEntryIris.map(navigationEntryIri => {
                ...
                const relatedEntityConfig =
                  Rdf.getValueFromPropertyPath<Rdf.Iri>([Rdf.iri('http://www.researchspace.org/pattern/system/resource_configuration/menu_item_config')], navigationEntryPg).get();

                const relatedKps =
                  Rdf.getValuesFromPropertyPath<Rdf.Iri>([Rdf.iri('http://www.researchspace.org/pattern/system/resource_configuration/is_related_by_kp')], navigationEntryPg).map(kpIri => kps[kpIri.value]);
