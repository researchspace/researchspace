...
export class LdpAnnotationServiceClass extends LdpService {
  ...
  public updateAnnotation(annotationIri: Rdf.Iri, annotation: Annotation): Kefir.Property<Rdf.Iri> {
    return this.serializeAnnotation(annotation)
      .flatMap((graph) => this.update(annotationIri, graph))
      .toProperty();
  }
  ...
  private serializeAnnotation(annotation: Annotation): Kefir.Property<Rdf.Graph> {
    ...
    return metadata.map((mGraph) => Rdf.union(mGraph, this.annotationToGraph(annotation)));
  }
  ...
  public getAnnotation(annotationIri: Rdf.Iri): Kefir.Property<Annotation> {
    return this.get(annotationIri).map((graph) => this.parseGraphToAnnotation(annotationIri, graph));
  }
      ...
      rdfa: graph.triples
        .filterNot(
          (t) =>
            ...
            !t.p.equals(vocabularies.rdfs.label) &&
            !t.p.equals(vocabularies.rdf.type) &&
            !t.p.equals(vocabularies.oa.hasTarget) &&
            !t.p.equals(vocabularies.oa.hasBody)
        )
        .map((t) => {
          return {
            predicate: t.p,
...
export var LdpAnnotationService = new LdpAnnotationServiceClass(rso.AnnotationsContainer.value);
