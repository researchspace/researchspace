...
import org.researchspace.federation.repository.evaluation.MpFederationStrategy;
import org.researchspace.federation.repository.evaluation.RemoteClosingExceptionConvertingIteration;
import org.researchspace.federation.repository.optimizers.MpFederationJoinOptimizer;
import org.researchspace.federation.repository.optimizers.MpFederationJoinOptimizerWithHints;
import org.researchspace.federation.repository.optimizers.MpFederationServiceClauseOptimizer;
import org.researchspace.federation.repository.optimizers.MpPostJoinReorderingLocalJoinOptimizer;
import org.researchspace.federation.repository.optimizers.MpPrepareOwnedTupleExpr;
import org.researchspace.federation.repository.optimizers.MpQueryHintsSyncOptimizer;
import org.researchspace.federation.repository.optimizers.MpQueryJoinOrderOptimizer;
import org.researchspace.federation.repository.optimizers.MpQueryMultiJoinOptimizer;
 ...
 * Assumes an explicit SPARQL 1.1. federation, and not a transparent one: i.e.,
 * there is one default federation member, while all others must be accessed via
 * SPARQL 1.1 SERVICE clauses.
 * 
...
public class MpFederationConnection extends AbstractFederationConnection {
    ...
    protected class MpFederationTripleSource implements TripleSource {
        ...
        public CloseableIteration<? extends Statement, QueryEvaluationException> getStatements(Resource subj, IRI pred,
            ...
            try {
                ...
                return new RemoteClosingExceptionConvertingIteration<Statement>(result);
    ...
    public MpFederationConnection(MpFederation federation, List<RepositoryConnection> members) {
    ...
    /**
     * For single-owner queries we aim not to touch the query tuple expression at
     * all. However, there can still be cases where this is necessary: e.g., if the
     * user added query hints, those query hints will not be understood by the
     * underlying repository and so have to be deleted (we don't apply them anyway).
     */
    ...
    protected TupleExpr optimize(TupleExpr parsed, Dataset dataset, BindingSet bindings, boolean includeInferred,
        ...
        if (queryHintsEnabled) {
            ...
            // Finally, we reorder the join operands later, at the end of the optimization
            // workflow
        ...
        // new EmptyPatternOptimizer(members, bloomFilterFunction).optimize(query,
        // dataset,
        // bindings);
        ...
        new MpFederationServiceClauseOptimizer(members, getDefaultMemberConnection(), mpFederation.getServiceMappings(),
                hintsSetup).optimize(query, dataset, bindings);
