...
export function getGraphDataWithLabels(
  ...
  const graphData = getGraphData(config, options);
  const labels = graphData.flatMap(fetchLabels(options));
  const thumbnails = graphData.flatMap(fetchThumbnails(options));
...
function fetchLabels(options?: SparqlClient.SparqlOptions) {
  return function(
    elements: ResourceCytoscapeElement[]
  ): Kefir.Property<Map<Rdf.Node, string>> {
    return fetchLabelsForResources(elements, options).map(
...
function getLabelsForLiterals(
  ...
  const elementsWithLiterals = filter(elements, e => e.data.node.isLiteral);
  return Map(
...
function fetchLabelsForResources(
  elements: ResourceCytoscapeElement[], options?: SparqlClient.SparqlOptions
  ...
  return getLabels(iris, {semanticContext: options});
}

/**
 * Fetch thumbnails for resources using [ThumbnailService].
 */
function fetchThumbnails(options?: SparqlClient.SparqlOptions) {
  return function(
    elements: ResourceCytoscapeElement[]
  ): Kefir.Property<Map<Rdf.Node, string>> {
    const elementsWithIri = filter(elements, e => e.group === 'nodes' && e.data.node.isIri());
    const iris = map(elementsWithIri, e => e.data.node as Rdf.Iri);
    return getThumbnails(iris, {semanticContext: options});
  };
}
...
export function getGraphData(
  ...
  return SparqlClient.construct(config.query, options).map(
