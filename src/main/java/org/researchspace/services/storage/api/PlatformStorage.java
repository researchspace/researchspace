...
/**
 * Represents a stack of registered {@link ObjectStorage} instances in fixed
 * override order.
 */
public interface PlatformStorage {
    ...
    /**
     * Creates and returns default metadata for current user (determined from thread
     * context).
     */
    ObjectMetadata getDefaultMetadata();

    /**
     * Searches registered storage instances for given object in reverse override
     * order, e.g. [overrideN, overrideN-1, ..., base], and returns the first match.
     */
    ...
    /**
     * Searches registered storage instances for all objects matching specified path
     * prefix; and returns first match for each object path using the same semantics
     ...
     * 
     * @throws IllegalArgumentException when storage with specified ID does not
     *                                  exists
     */
    @NotNull
     ...
     * 
     * @return storage status list in override order, e.g. [base, override1,
     *         override2, ...]
     */
