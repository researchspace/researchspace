...
public class ShiroTextRealm extends IniRealm {
    ...
    private Ini getIniFromConfig(Configuration config) {
        SecurityConfigRecord record = config.getEnvironmentConfig().getSecurityConfig(SecurityConfigType.ShiroConfig);
        ...
        try (InputStream stream = record.readStream()) {
            ...
            isConfigShiro = false;
            logger.error("Not able to load {} from {}", record.getType().getFileName(),
    ...
    private void saveIni(Ini ini) {
        SecurityConfigRecord record = config.getEnvironmentConfig().getSecurityConfig(SecurityConfigType.ShiroConfig);
        logger.trace("Persisting changes to accounts. Saving changes to SHIRO text realm: "
                + record.getLocationDescription());
