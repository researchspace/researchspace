...
import { xsd, rdfs } from 'platform/api/rdf/vocabularies';
...
<html dir="ltr" >
  <head>
    ...
    <script>
      ...
      window.rsPdfJs.annotations=${JSON.stringify(snippets)};
    </script>
...
const P138i_has_representation = createDirectField(crm.P138i_has_representation, {
    id: 'P138i_has_representation',
    xsdDatatype: xsd.anyURI,
});


const label = createDirectField(rdfs.label, {
    id: 'label',
    xsdDatatype: xsd._string,
});
...
class PdfAnnotator extends Component<PdfAnnotatorProps, PdfAnnotatorState> {
    ...
    componentDidMount() {
        ...
        this.fileManager = new FileManager({ repository });

        // get document file
        const query = SparqlUtil.parseQuery(
            `
SELECT ?file {
  ?document crm:P1_is_identified_by ?file .
  ?file a rso:EX_File
}
      `
        );
        this.cancel.map(
            SparqlClient.select(
                ...
                res => this.fileManager.getFileResource(res.results.bindings[0].file as Rdf.Iri, { namePredicateIri: 'http://www.researchspace.org/ontology/PX_has_file_name', mediaTypePredicateIri: 'http://www.researchspace.org/ontology/PX_has_media_type' })
            ).map(
    ...
    private getDocumentPages() {
        const query = SparqlUtil.parseQuery(
            `
SELECT ?expression {
  ?document crm:P165_incorporates ?expression .
}
      `
        );
        this.cancel.map(
            ...
            res => {
                this.setState({ expressionIri: res.results.bindings[0].expression as Rdf.Iri });
            }
        );
    }
...
SELECT ?page ?pageNumber ?snippet ?text ?snippetType ?position ?sortIndex {
  ?page crm:P2_has_type <https://researchspace.org/resource/vocab/researchspace_publication_structure/Page> .
  ?page frbroo:R5i_is_component_of ?document ;
        crm:P1_is_identified_by ?pageIdentifier .
  ?pageIdentifier crm:P2_has_type <https://researchspace.org/resource/vocab/identifier_types/PDF_Page> ;
    crm:P190_has_symbolic_content ?pageNumber .
  ...
  OPTIONAL {
    ...
    render() {
        ...
        if (this.state.pdfUrl && this.state.snippets && this.state.expressionIri) {
            return (
                <Frame
                    initialContent={
                ...
                if (event.detail.editorId === this.editorId) {
                    ...
                    if (!this.state.snippets.some(s => s.id === annotation.id)) {
                        ...
                        if (annotation.type == 'image') {
                            const blob = new Blob([this.convertBase64ImageToBlob(annotation.image)]);
                            const file = new File([blob], `snippet-${annotation.id}.png`);
                            const upload = this.fileManager.uploadFileAsResource({
                                file,
                                storage: this.props.storage,
                                resourceQuery: `
  CONSTRUCT {
    ?__resourceIri__ a <http://www.researchspace.org/ontology/EX_File>.
    ?__resourceIri__ <http://www.researchspace.org/ontology/PX_has_file_name> ?__fileName__.
    ?__resourceIri__ <http://www.researchspace.org/ontology/PX_has_media_type> ?__mediaType__.
  } WHERE {}
                        ...
                        `
                            ...
                            saveAction = this.cancel.map(upload).flatMap(fileIri => this.saveAnnotation(annotation, fileIri));
                        } else {
                            saveAction = this.saveAnnotation(annotation);
                        }
    ...
    private saveAnnotation(annotation: any, imageFileIri?: Rdf.Iri) {
        const pageIri = `${this.props.documentIri}/page/${annotation.position.pageIndex + 1}`;
        ...
        const modelKps = [
            { def: RdfType, value: valueFromRdf(Rdf.iri('http://iflastandards.info/ns/fr/frbr/frbroo/F22_Self-Contained_Expression')) },
            {
                ...
                value: makeComposite(
                    ...
                    [
                        {
                            def: RdfType,
                            value: valueFromRdf(Rdf.iri('http://iflastandards.info/ns/fr/frbr/frbroo/F22_Self-Contained_Expression'))
                        },
                        {
                            def: R5iIsComponentOf,
                            value: [valueFromRdf(Rdf.iri(this.props.documentIri)), valueFromRdf(this.state.expressionIri)]
                        },
                        {
                            def: P2_has_type, value: valueFromRdf(Rdf.iri('https://researchspace.org/resource/vocab/researchspace_publication_structure/Page'))
                        },
                        {
                            def: label,
                            value: valueFromRdf(Rdf.literal(`page ${annotation.position.pageIndex + 1}`))
                        },
                        {
                            def: P1_is_identified_by,
                            value: [
                                makeComposite(
                                    null, `${pageIri}/id/pdf_id`, [
                                    {
                                        def: RdfType, value: valueFromRdf(crm.E42_Identifier),
                                    },
                                    {
                                        def: P2_has_type, value: valueFromRdf(Rdf.iri('https://researchspace.org/resource/vocab/identifier_types/PDF_Page'))
                                    },
                                    {
                                        def: P190_has_symbolic_content,
                                        value: valueFromRdf(Rdf.literal(`${annotation.position.pageIndex + 1}`))
                                    },
                                ]
                                ),
                                makeComposite(
                                    null, `${pageIri}/id/expression_id`, [
                                    {
                                        def: RdfType, value: valueFromRdf(crm.E42_Identifier),
                                    },
                                    {
                                        def: P2_has_type, value: valueFromRdf(Rdf.iri('https://researchspace.org/resource/vocab/identifier_types/Expression_Page'))
                                    },
                                    {
                                        def: P190_has_symbolic_content,
                                        value: valueFromRdf(Rdf.literal(`${annotation.pageLabel}`))
                                    },
                                ]
                                )
                            ]
            ...
            },
            {
                ...
                value: makeComposite(
                    ...
                    {
                        def: P2_has_type, value: valueFromRdf(Rdf.iri('https://www.zotero.org/' + annotation.type))
                    },
                    {
                        def: P190_has_symbolic_content,
        ...
        if (annotation.type === 'highlight') {
            modelKps.push(
                ...
                {
                    def: P2_has_type, value: valueFromRdf(Rdf.iri('https://researchspace.org/resource/vocab/researchspace_bibliography_types/Text_Content'))
                },
            );
        } else {
            modelKps.push(
                {
                    def: P2_has_type, value: valueFromRdf(Rdf.iri('https://researchspace.org/resource/vocab/researchspace_bibliography_types/Area_Content'))
                },
                {
                    def: label,
                    value: valueFromRdf(Rdf.literal(`clipping from page ${annotation.position.pageIndex + 1}`))
                },
                {
                    def: P138i_has_representation,
                    value: makeComposite(
                        null, `${snippetIri}/image`, [
                        {
                            def: RdfType, value: valueFromRdf(Rdf.iri('http://www.researchspace.org/ontology/EX_Digital_Image')),
                        },
                        {
                            def: P1_is_identified_by,
                            value: valueFromRdf(imageFileIri)
                        },
                        {
                            def: label,
                            value: valueFromRdf(Rdf.literal(`image representation of the clipping from page ${annotation.position.pageIndex + 1}`))
                        },
                    ]
                    )
                },
        ...
        return this.cancel.map(
            this.getPersistence().persist(FieldValue.empty, snippetModel)
        );
    }

    /**
     * zotero annotator gives us image as base64 encoded string, to upload it we need to convert it to js Blob
     * see also https://stackoverflow.com/questions/16245767/creating-a-blob-from-a-base64-string-in-javascript
     */
    private convertBase64ImageToBlob(base64Image: string): Blob {
