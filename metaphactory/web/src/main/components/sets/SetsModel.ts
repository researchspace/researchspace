...
function querySetItems(
  ...
  try {
    ...
    const preferredLabels = ConfigHolder.getUIConfig().preferredLabels.value;
    new PropertyPathBinder({[SetItemsBindings.PreferredLabel]: {
      type: 'path',
      pathType: '|',
      items: preferredLabels.map(iri => Rdf.fullIri(iri).value) as SparqlJs.Term[],
    }}).sparqlQuery(parsedQuery);
