...
public class VirtuosoKeywordSearchHandler implements KeywordSearchHandler {
    ...
    protected static class VirtuosoSparqlFullTextSearchSerializerQueryVisitor
        ...
        protected void renderKeywordSearchUsingContains(KeywordSearchPattern pattern) {
            ...
            pattern.getMatchVar().visit(this);
            builder.append(" ");
            TupleExprs.createConstVar(VirtuosoKeywordSearchSail.BIF_CONTAINS).visit(this);
            builder.append(" \"");
            builder.append(String.join(" and ", cleanedTokens));
            builder.append("\"");
    ...
    protected String generateTargetQuery(TupleExpr tupleExpr, ParsedQueryPreprocessor preprocessor,
        ...
        StringBuilder queryBuilder = new StringBuilder("SELECT * WHERE { { ");
        queryBuilder.append(serializer.serialize(toSerialize));
        queryBuilder.append("} } ");
        if (serializer.limit != -1) {
            queryBuilder.append(" LIMIT " + serializer.limit);
        }
