...
public class ShaclUtils {
    ...
    private static final Logger logger = LogManager.getLogger(ShaclUtils.class);

    private ShaclUtils() {
    }

    public static void expandShapesFromQueryTemplates(RepositoryConnection conn, Repository assetsRepository) {

        List<Statement> stmts = Iterations.asList(conn.getStatements(null, MPQA.hasSPINQueryTemplate, null));
        ...
        try (RepositoryConnection assetsConnection = assetsRepository.getConnection()) {
            stmts.stream().forEach(stmt -> {
                ...
                List<Statement> queryBodyStmts = Iterations
                        .asList(assetsConnection.getStatements(queryIRI, SPIN.BODY_PROPERTY, null));
                if (queryBodyStmts.isEmpty()) {
                    throw new IllegalStateException(
                            "Referenced query template " + queryIRI.stringValue() + " does not have a body");
                }

                queryBodyStmts.stream().findFirst().ifPresent(stmtQueryBody -> {
                    Iterations.asList(assetsConnection.getStatements((Resource) stmtQueryBody.getObject(),
                            SP.TEXT_PROPERTY, null)).stream().findFirst().ifPresent(stmtQueryText -> {
                                toAdd.add(
                                        VF.createStatement(stmt.getSubject(), SHACL.select, stmtQueryText.getObject()));
                            });
                });
            });
        }
    ...
    private static void putStatementsIntoModelRecursively(RepositoryConnection conn, Resource root, Model model) {
        ...
        objectResources.stream().forEach(res -> putStatementsIntoModelRecursively(conn, res, model));
    }
    ...
    private static void putStatementsIntoModelRecursively(Model parentModel, Resource root, Model model) {
        ...
        objectResources.stream().forEach(res -> putStatementsIntoModelRecursively(parentModel, res, model));
    }
    ...
    /**
     * Retrieves from the model all objects of the property <code>property</code>
     * and (optionally) subject <code>subject</code> where these objects are
     * resources
     * 
     * @param model
     * @param property
    ...
    public static List<Resource> getObjectResources(Model model, IRI property, Resource subject) {
        ...
        return model.filter(subject, property, null).stream().map(stmt -> stmt.getObject())
                .filter(val -> (val instanceof Resource)).map(val -> (Resource) val).collect(Collectors.toList());
    }

    public static void expandWithReferencedQueryTemplates(Model model, LocalLDPAPIClient ldpApiClient)
            throws Exception {

        List<Resource> templateReferences = model.filter(null, MPQA.hasSPINQueryTemplate, null).stream()
                .map(stmt -> stmt.getObject()).filter(val -> (val instanceof Resource)).map(val -> (Resource) val)
        ...
        for (Resource ref : templateReferences) {
            try {
                Model templateModel = ldpApiClient.getObjectModel(ref);
                ...
                logger.warn("Could not retrieve the query template <" + ref.stringValue() + ">: " + e.getMessage());
    ...
    public static List<QueryTemplate<?>> retrieveQueryTemplates(Model parentModel) throws Exception {
        List<Resource> referencedTemplateResources = parentModel.filter(null, MPQA.hasSPINQueryTemplate, null).stream()
                .map(stmt -> stmt.getObject()).filter(val -> (val instanceof Resource)).map(val -> (Resource) val)
                .collect(Collectors.toList());
