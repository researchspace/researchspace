...
export class DatePickerInput extends AtomicValueInput<DatePickerInputProps, {}> {
  ...
  private handleKeyboardInput(value: string): AtomicValue | EmptyValue {
    ...
    const formatMap: { [key: string]: { momentFormat: string; unit?: StartOf } } = {
      'YYYY': { momentFormat: 'YYYY', unit: 'year' },
      'YYYY-MM': { momentFormat: 'YYYY-MM', unit: 'month' },
      'MM/YYYY': { momentFormat: 'MM/YYYY', unit: 'month' },
      'MM-YYYY': { momentFormat: 'MM-YYYY', unit: 'month' },
      'DD/MM/YYYY': { momentFormat: 'DD/MM/YYYY' },
    };

    const formatInfo = formatMap[detectedFormat];
    const momentValue = moment(value, formatInfo.momentFormat, true); // Strict parsing
    ...
    if (formatInfo.unit) {
      if (this.end) {
        momentValue.endOf(formatInfo.unit);
      } else {
        momentValue.startOf(formatInfo.unit);
      }
    }
    ...
    const convertedValue = momentValue.format(OUTPUT_UTC_DATE_FORMAT);
  ...
  private utcMomentAsIfItWasLocal(utcMoment: Moment | undefined): Moment | undefined {
    ...
    // We need to create a local moment with the same date parts as the UTC moment.
    // The easiest way to do this is to format the UTC moment as a string and then parse it as a local moment.
    const format = this.mode === 'date' ? OUTPUT_UTC_DATE_FORMAT : this.mode === 'time' ? OUTPUT_UTC_TIME_FORMAT : 'YYYY-MM-DDTHH:mm:ss';
    const formatted = utcMoment.format(format);
    return moment(formatted, format);
  }

  private localMomentAsIfItWasUtc(localMoment: Moment) {
    // The date picker returns a local moment. We want to treat it as if it was a UTC moment.
    // The easiest way to do this is to format the local moment as a string and then parse it as UTC.
    const format = this.mode === 'date' ? OUTPUT_UTC_DATE_FORMAT : this.mode === 'time' ? OUTPUT_UTC_TIME_FORMAT : 'YYYY-MM-DDTHH:mm:ss';
    const formatted = localMoment.format(format);
...
export function utcMomentFromRdfLiteral(literal: Rdf.Literal | undefined): Moment | undefined {
  ...
  // We need to parse the literal's value as a UTC moment.
  // The format of the value can be one of several formats, so we need to try them all.
  const formats = [
    INPUT_XSD_DATETIME_FORMAT,
    INPUT_XSD_DATE_FORMAT,
    OUTPUT_UTC_DATE_FORMAT, // This is for dates that are already in the correct format.
    INPUT_XSD_TIME_FORMAT,
    OUTPUT_UTC_TIME_FORMAT, // This is for times that are already in the correct format.
  ];

  const parsedMoment = moment.utc(literal.value, formats, true);
