...
public class ModelToQueryTransformer implements ModelToDtoTransformer<Query<?>> {
	...
	public Query<?> modelToDto(final Model model) throws InvalidQueryModelException {
		...
		/**
		 * This is a naive implementation for now, but at the time being
		 * we don't expect any performance problems here.
		 */
		IRI queryId = null;
		IRI queryType = null;
		String label = null;
		...
		if (queryType.equals(SP.SELECT_CLASS)) {
			...
			ret = new SelectQuery(queryId, label, description, queryString);
			
		} else if (queryType.equals(SP.ASK_CLASS)) {
			
			ret = new AskQuery(queryId, label, description, queryString);

		} else if (queryType.equals(SP.CONSTRUCT_CLASS)) {
			
			ret = new ConstructQuery(queryId, label, description, queryString);
			
		} else if (queryType.equals(SP.DESCRIBE_CLASS)) {

			ret = new DescribeQuery(queryId, label, description, queryString);
