...
import * as ConfigHolder from 'platform/api/services/config-holder';
...
/*
 * Intercept calls to document.registerElement to track all custom components.
 */
const registerElement = document.registerElement;
document.registerElement = function(name: string, opts: {}) {
...
export function parseHtmlToReact(
  ...
  const processingInstructions: Instruction[] = [
    ...
    {
      shouldProcessNode: TemplateParser.isTemplate,
      processNode: skipNode,
    },
    {
  ...
  const htmlToReactParser = new ToReactParser(React);
...
function isNativeComponent(node: Node): boolean {
  ...
  // see https://developer.mozilla.org/en-US/docs/Web/API/Document/registerElement
...
function processCode(isCodeExample: boolean) {
  return function(
    ...
    const codeComponent = isCodeExample ? 'mp-code-example' : 'mp-code-highlight';
    const codeToHighlight = isCodeExample ?
      html(innerCode, {wrap_attributes: ('force-expand-multiline' as any)}) : innerCode;
...
function processStyle(
  ...
  return Promise.resolve(
    React.DOM.style({dangerouslySetInnerHTML: { __html: node.children[0].data}}, null )
  );
}
...
function createStyleJsonFromString(styleString: string): CSSProperties {
  if (!styleString) {
    return {};
  }
