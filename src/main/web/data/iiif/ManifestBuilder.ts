...
function processJsonResponse(response: {}): Kefir.Stream<Manifest> {
  ...
  return JsonLd.frame(response, MANIFEST_FRAME, { documentLoader })
    ...
    .flatMap((framed) => {
      return JsonLd.compact(framed, MANIFEST_FRAME, { documentLoader }).mapErrors(
        (compactError) => new ManifestBuildingError('Failed to compact JSON-LD', compactError)
      );
    });
}
...
exif:height ?canvasHeight.
  ...
  return Kefir.zip([queryingDisplayLabel, queryingImagesLabels])
    ...
    .map((query) => SparqlUtil.serializeQuery(query))
    .toProperty();
}
