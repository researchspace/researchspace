...
describe('Plain Text Component', () => {
  ...
  describe('languages', () => {
    ...
    it('render language select list', () => {
      expect(inputWithLang.find('Select')).to.have.length(1);
    });

    it('without language select when languages not exist', () => {
      expect(inputComponent.find('Select')).to.not.have.length(1);
    });

    it('show empty language when default don\'t exist', () => {
      expect(inputWithLang.find('Select').props().value).to.eql({
        key: undefined,
        label: 'No language',
      });
    });
    ...
    it('show default language when its langLiteral', () => {
      ...
      const wrapper = mount(createElement(PlainTextInput, props));
      expect(wrapper.find('Select').props().value).to.eql({key: 'language', label: 'language'});
    });
    ...
    it('show "No language" if literal', () => {
      ...
      expect(
        literalInput.find('Select').props().value
      ).to.eql({
        key: undefined,
        label: 'No language',
      });
