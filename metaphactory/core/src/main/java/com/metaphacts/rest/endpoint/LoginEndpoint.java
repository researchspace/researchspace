...
public class LoginEndpoint {
    ...
    public String getLoginPageGet(@Context HttpServletRequest httpRequest) {
    ...
    @Path("{path: .*}")
    @Produces(MediaType.TEXT_HTML)
    public String getLoginPagePost(@Context HttpServletRequest httpRequest) {
        return getPage(httpRequest);
    }
    
    private String getPage(HttpServletRequest request){
        try {
            Map<String, Object> map = st.getConfigurationPropertyMap();
            ...
            return st.getTemplateFromAppOrClasspath(TEMPLATES.LOGIN_PAGE, map);
        } catch (Exception e) {
