...
/**
 * This optimizer was added to perform additional post-processing step to merge
 * join operands which can be sent together to a remote endpoint. It could
 * happen that we had join operands, some of which were affected by a query
 * hint. For example: :a rdfs:label ?label . :a :myValueCode ?code .
 * ephedra:Prior ephedra:executeFirst "true" . SERVICE :blah { ?results :hasCode
 * ?code . } In this case, Ephedra cannot merge the first two operands in
 * {@link MpFederationJoinOptimizerWithHints}: one of them is affected by a
 * query hint. Afterwards, in {@link MpQueryJoinOrderOptimizer}, these two
 * operands are reordered according to the query hint. Only now we can go
 * through the join operands again and merge them.
 * 
 * @author Andriy Nikolov an@metaphacts.com
 *
 */
public class MpPostJoinReorderingLocalJoinOptimizer extends AbstractQueryModelVisitor<RepositoryException>
        ...
        for (int i = 1; i < args.size(); i++) {
            ...
            if ((previousOperand instanceof MpOwnedTupleExpr) && (currentOperand instanceof MpOwnedTupleExpr)) {
                ...
                if ((prevOwner != null) && (currentOwner != null) && (prevOwner.equals(currentOwner))) {
                    previousOperand = merge((MpOwnedTupleExpr) previousOperand, (MpOwnedTupleExpr) currentOperand);
