...
public class LDPImplManager {
    ...
    public static LDPResource getLDPImplementation(IRI iri, Set<IRI> types, MpRepositoryProvider repositoryProvider) {
        ...
        if (knownContainerImplementations.containsKey(iri))
            ...
            cl = isContainer(iri, types) ? getContainerImplementation(iri, types)
                    : getResourceImplementation(iri, types);

        logger.trace("Selected implementation for LDP Container: " + cl);
        try {
            @SuppressWarnings("unchecked")
            Constructor<LDPResource> cons = (Constructor<LDPResource>) cl.getConstructor(IRI.class,
                    MpRepositoryProvider.class);
    ...
    private static boolean isContainer(IRI iri, Set<IRI> types) {
        boolean b = RootContainer.IRI.equals(iri) || !Collections.disjoint(types,
                Sets.newHashSet(LDP.Container, LDP.BasicContainer, LDP.DirectContainer));
        if (logger.isTraceEnabled())
            logger.trace("Resource is a LDP Container: " + b);
        return b;
    }

    private static Class<? extends LDPResource> getContainerImplementation(IRI iri, Set<IRI> types) {
        Class<? extends LDPContainer> impl = knownContainerImplementations.containsKey(iri)
                ? knownContainerImplementations.get(iri)
                : null;
        if (impl != null)
            return impl;
        for (Resource r : types) {
            if (knownContainerImplementations.containsKey(r))
                return knownContainerImplementations.get(r);
        }
    ...
    private static Class<? extends LDPResource> getResourceImplementation(IRI iri, Set<IRI> types) {
        for (Resource r : types) {
            if (knownResourceImplementations.containsKey(r))
                return knownResourceImplementations.get(r);
        }
    ...
    private static Map<IRI, Class<? extends LDPResource>> listLDPImplementations() {
        ...
        Set<Class<?>> classes = findLdpImplementations(
                classInfo -> classInfo.implementsInterface(LDPResource.class.getName())
    ...
    private static Map<IRI, Class<? extends LDPContainer>> listLDPContainerImplementations() {
        ...
        Set<Class<?>> classes = findLdpImplementations(
                classInfo -> classInfo.implementsInterface(LDPContainer.class.getName()));
        for (Class<?> cl : classes) {
            LDPR a = cl.getAnnotation(LDPR.class);
