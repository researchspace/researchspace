...
import { createFactory, createElement, DOM as D, FormEvent, ReactElement } from 'react';
...
export class QueryTemplate extends Component<QueryTemplateProps, State> {
    ...
    this.queryTemplateService
      ...
      .onValue(({template, queryIri, templateCount}) => {
        ...
        getLabels(template.categories).map(
          labels => _.map(labels.toObject(), (v, k) =>
            ({iri: Rdf.iri(k), label: Rdf.iri(v) })
          )
        ).onValue(categories =>
  ...
  private updateQuery = (): Kefir.Property<string> => {
