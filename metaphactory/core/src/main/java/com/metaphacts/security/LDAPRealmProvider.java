...
public class LDAPRealmProvider implements Provider<Realm> {
    ...
    public Realm get() {
           ...
           Ini ini  = Ini.fromResourcePath(config.getEnvironmentConfig().getShiroLDAPConfig());
           logger.info("Trying to intialize SHIRO LDAP realm from: " + config.getEnvironmentConfig().getShiroLDAPConfig());
           ...
           if(r==null || !JndiLdapRealm.class.isAssignableFrom(r.getClass()))
               throw new IllegalArgumentException(
                   config.getEnvironmentConfig().getShiroLDAPConfig() 
                   + " does not specify a valid JndiLdapRealm.");
