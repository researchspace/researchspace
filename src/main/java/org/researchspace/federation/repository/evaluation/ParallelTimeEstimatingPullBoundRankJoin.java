...
import org.researchspace.federation.sparql.rank.optimizers.NaryJoinOrderOptimizerHelper;
...
/**
 * Implementation of the parallel competing pull-bound rank join (see FedSearch
 * paper).
 * 
...
public class ParallelTimeEstimatingPullBoundRankJoin {
    ...
    // protected Map<BindingSet, List<BindingSet>> leftHashMap = new
    // HashMap<BindingSet,
    // List<BindingSet>>();
    // protected Map<BindingSet, List<BindingSet>> rightHashMap = new
    // HashMap<BindingSet,
    // List<BindingSet>>();
    ...
    public ParallelTimeEstimatingPullBoundRankJoin(MpFederationStrategy strategy, List<TupleExpr> seeds,
            List<TupleExpr> unsortedArgs, BindingSet bindings, List<OrderElem> orderElements, long limit) {
        ...
        this.orderComparator = new MpOrderComparator(strategy, orderElements, new ValueComparator());
        this.limit = limit;
    ...
    public CloseableIteration<BindingSet, QueryEvaluationException> evaluate(MpFederationStrategy strategy,
            List<TupleExpr> seeds, List<TupleExpr> unsorted, BindingSet bindings) {
        ...
        for (TupleExpr seed : seeds) {
            PipelineJoinQueueEvaluator evaluator = new PipelineJoinQueueEvaluator(this, strategy, seed, null, seed,
                    bindings, freeVarsSetsList.get(seed));
        ...
        if (seeds.size() == 2) {
            ...
            try {
                MaterializedIteration iter1 = (MaterializedIteration) partialIterations.get(seeds.get(0));
                MaterializedIteration iter2 = (MaterializedIteration) partialIterations.get(seeds.get(1));
                CollectionIteration<BindingSet, QueryEvaluationException> colIter1 = new CollectionIteration<>(
                        iter1.list);
                CollectionIteration<BindingSet, QueryEvaluationException> colIter2 = new CollectionIteration<>(
                        iter2.list);
                ...
                return new HashJoinIteration(strategy, colIter1, iter1.list.get(0).getBindingNames(), colIter2,
    ...
    protected void initializeQueues() {
        ...
        for (int i = 0; i < this.seeds.size(); i++) {
            ...
            this.freeVarsSetsList.put(seeds.get(i), Sets.newHashSet((seeds.get(i).getBindingNames())));
    ...
    protected List<TupleExpr> generatePossibleJoinQueueForSortedInput(TupleExpr expr) {
        ...
        List<TupleExpr> optimized = new NaryJoinOrderOptimizerHelper(Lists.newArrayList(this.unjoinedUnsortedArgs),
                expr.getBindingNames()).orderJoinOperands();
    ...
    public void operationFinishCallback(PipelineJoinQueueEvaluator evaluator, TupleExpr justJoined,
        ...
        log.debug("Just joined: " + justJoined.toString() + " to branch " + evaluator.getSeed().toString());
        ...
        synchronized (this) {
            ...
            if (!ourQueue.isEmpty()) {
                PipelineJoinQueueEvaluator newEvaluator = new PipelineJoinQueueEvaluator(this, strategy,
                        evaluator.getSeed(), result, ourQueue.get(0), new MapBindingSet(), branchVars);
