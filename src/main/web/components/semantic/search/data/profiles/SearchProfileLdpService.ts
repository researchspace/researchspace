...
export class SearchProfileProfileLdpServiceClass extends LdpService {
  ...
  getProfile(iri: Rdf.Iri): Kefir.Property<Profile> {
    return this.get(iri).map((graph) => this.parseGraphToProfile(iri, graph));
  }
  ...
  private profileToGraph(iri: Rdf.Iri, profile: CategoryProfile): Rdf.Graph {
    const base = Rdf.graph([
      ...
      Rdf.triple(iri, rdfs.comment, Rdf.literal(profile.description)),
    ]).triples;
    ...
    return Rdf.graph(base.union(categories).union(relations));
  }

  private categoriesToGraph(iri: Rdf.Iri, categories: Categories): Set<Rdf.Triple> {
    return this.collectionToGraph(iri, categories, StructuredSearchVocab.hasCategory, StructuredSearchVocab.category);
  }

  private relationsToGraph(iri: Rdf.Iri, relations: Relations): Set<Rdf.Triple> {
    return this.collectionToGraph(
      iri,
      relations.mapKeys((key) => key.value.iri).toOrderedMap(),
      StructuredSearchVocab.hasRelation,
      StructuredSearchVocab.relation
    );
  }

  private collectionToGraph(
    iri: Rdf.Iri,
    collection: OrderedMap<Rdf.Iri, Resource>,
    hasIri: Rdf.Iri,
    relationIri: Rdf.Iri
  ): Set<Rdf.Triple> {
    return collection
      ...
      .map((category, i) => {
        ...
        return Set.of(
          Rdf.triple(iri, hasIri, holder),
          Rdf.triple(holder, relationIri, category.iri),
          Rdf.triple(holder, StructuredSearchVocab.order, Rdf.literal(i.toString(), xsd.integer))
        );
      })
      .flatten()
      .toSet();
  ...
  private getProfileMetadata(profileIri: Rdf.Iri, graph: Rdf.Graph): { label: string; description: string } {
    return {
      label: graph.triples.find((t) => t.s.equals(profileIri) && t.p.equals(rdfs.label)).o.value,
      description: graph.triples.find((t) => t.s.equals(profileIri) && t.p.equals(rdfs.comment)).o.value,
    };
  }

  private getCategories(graph: Rdf.Graph): OrderedSet<Rdf.Iri> {
    const categoryHolders = graph.triples.filter((t) => t.p.equals(StructuredSearchVocab.hasCategory)).map((t) => t.o);
    ...
    return OrderedSet<Rdf.Iri>(
      categoryHolders
        .map((categoryNode) => {
          ...
          const order = graph.triples.find((t) => t.s.equals(categoryNode) && t.p.equals(StructuredSearchVocab.order))
            .o;
          return {
            order: order,
            value: category,
  ...
  private getRelations(graph: Rdf.Graph): OrderedSet<Rdf.Iri> {
    const categoryHolder = graph.triples.filter((t) => t.p.equals(StructuredSearchVocab.hasRelation)).map((t) => t.o);
...
export const SearchProfileLdpService = new SearchProfileProfileLdpServiceClass(
  StructuredSearchVocab.ProfileContainer.value
);
