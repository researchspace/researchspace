...
/**
 * This class processes a {@link SerializableParsedTupleQuery} and renders it as
 * a SPARQL string.
 * 
...
public class PreprocessedQuerySerializer extends AbstractQueryModelVisitor<RuntimeException> {
    ...
    /**
     * Enumeration of standard SPARQL 1.1 functions that are neither recognized by
     * RDF4J as special value expressions nor defined as IRI functions in the
     * <i>fn:</i> namespace (see {@link FNFunction}).
     * 
     */
     ...
     * 
     * @param query a parsed tuple query previously produced by
     *              {@link ParsedQueryPreprocessor}
    ...
    public void meet(Extension node) throws RuntimeException {
        ...
        for (ExtensionElem element : node.getElements()) {
            if (!isTautologicalExtensionElem(element) && !isExtensionElemAlreadyRendered(element)) {
                builder.append("\tBIND (");
                element.visit(this);
                builder.append(") . \n");
            }
    ...
    public void meet(If node) throws RuntimeException {
        writeAsFunction("IF", Lists.newArrayList(node.getCondition(), node.getResult(), node.getAlternative()));
    }
    ...
    public void meet(MultiProjection node) throws RuntimeException {
        ...
        if (node.getArg() instanceof Extension) {
            ...
            ext.getElements().stream().filter(elem -> (elem.getExpr() instanceof ValueExpr))
                    .forEach(elem -> valueMap.put(elem.getName(), elem.getExpr()));
        }
    ...
    public void meet(Regex node) throws RuntimeException {
        writeAsFunction("REGEX", Lists.newArrayList(node.getLeftArg(), node.getRightArg(), node.getFlagsArg()));
    }
    ...
    /**
     * A special case check: we project a variable from a subquery that has the same
     * name We must avoid writing SELECT (?x as ?x) WHERE { { SELECT ?x WHERE { ...
     * } } }
     * 
     */
