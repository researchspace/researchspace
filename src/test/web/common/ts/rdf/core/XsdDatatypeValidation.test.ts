...
const { parseXsdDatatype, validate, sameXsdDatatype, equal, datatypeEqual, datatypeToString } = XsdDataTypeValidation;
...
describe('XsdDatatypeValidation', () => {
  describe('parseXsdDatatype', () => {
    ...
    it('should parse datatype namespace', () => {
      ...
      expect(parseXsdDatatype(DATATYPE)).to.be.eql(<XsdDataTypeValidation.Datatype>{
    ...
    it('should parse string in short form for rdf', () => {
      expect(parseXsdDatatype(RDF_FULL_FORM)).to.be.eql(<XsdDataTypeValidation.Datatype>{
  ...
  describe('equal', () => {
    it('should return true for same datatypes', () => {
      expect(equal(Rdf.literal(SHORT_STRING), Rdf.literal(SHORT_STRING))).to.be.eql({ success: true });
    });

    it('should return error for different datatypes', () => {
      expect(equal(Rdf.literal(SHORT_STRING), Rdf.literal('xsd:date'))).to.be.eql(<
        XsdDataTypeValidation.ValidationResult
      >{
        success: false,
  ...
  describe('datatypeEqual', () => {
    describe('for string', () => {
      ...
      it('should return true for for same values', () => {
        expect(datatypeEqual(datatype, 'true', 'true')).to.be.eql(VALID_RESPONSE);
      });
  ...
  describe('datatypeToString', () => {
    it('should return string', () => {
      expect(datatypeToString(Rdf.iri(FULL_FORM))).to.be.eql(SHORT_STRING);
    });
  });
});
