...
import org.researchspace.keyword.KeywordSearchHandler;
...
import org.researchspace.sparql.renderer.SerializableParsedConstructQuery;
import org.researchspace.sparql.renderer.SerializableParsedTupleQuery;

import com.google.common.collect.Lists;

/**
 * Implementation of {@link KeywordSearchHandler} to process full-text search
 * queries over a Virtuoso triple store.
 * 
 * <p>
 ...
 * 
 * <p>
 * rendered as ?subj rdfs:label ?label . ?label bif:contains "'cation*'"
 * OPTION(score ?sc) .
 * </p>
 * 
 * <p>
 * Example search request with multiple predicates and a short token: query:
 * "ca*" score: yes predicate: rdfs:label, skos:prefLabel
 * </p>
 * 
 * <p>
 * <b>Deprecated</b> rendered using a bif:contains expression without a
 * wildcard: ?subj ?pred ?label . ?label bif:contains "'ca'" OPTION(score ?sc) .
 * FILTER (?pred = rdfs:label || ?pred = skos:prefLabel)
 * </p>
 * 
 * <p>
 * <b>Deprecated</b> Example search request with multiple predicates and a short
 * token: query: "ca*" score: yes predicate: rdfs:label, skos:prefLabel
 * </p>
 * 
 * <p>
 * <b>Deprecated</b> rendered using a LIKE filter (scores are not returned by
 * the backend): ?subj ?pred ?label . FILTER (?pred = rdfs:label || ?pred =
 * skos:prefLabel) FILTER (?label LIKE "ca%") BIND("1.0"^^xsd:double as ?score)
 * </p>
 * 
...
public class VirtuosoKeywordSearchHandler implements KeywordSearchHandler {
    ...
    public static final IRI BIF_CONTAINS = SimpleValueFactory.getInstance().createIRI("bif:contains");
    public static final IRI BIF_SCORE = SimpleValueFactory.getInstance().createIRI("bif:score");

    private static final ValueFactory VF = SimpleValueFactory.getInstance();
    public static final String PREDICATE_VAR_NAME = "pred";

    /**
     * Special implementation of the {@link PreprocessedQuerySerializer} that
     * renders a {@link TupleExpr} that can contain an instance of
     * {@link KeywordSearchGroupTupleExpr}. Such {@link KeywordSearchGroupTupleExpr}
     * will be rendered according to the Virtuoso full-text search syntax.
     *
     */
    protected static class VirtuosoSparqlFullTextSearchSerializerQueryVisitor extends PreprocessedQuerySerializer {
    ...
    public CloseableIteration<? extends BindingSet, QueryEvaluationException> evaluateKeywordSearchQuery(
        ...
        VirtuosoSparqlFullTextSearchSerializerQueryVisitor serializerVisitor = new VirtuosoSparqlFullTextSearchSerializerQueryVisitor();
    ...
    public String generateTargetQuery(ParsedQuery parsedQuery) {
        if (parsedQuery instanceof ParsedTupleQuery) {
            ...
            throw new IllegalArgumentException("Unsupported query type: " + parsedQuery.getClass().getName());
        }
    }
    ...
    public String generateTargetQuery(ParsedGraphQuery parsedQuery) {
        ParsedQueryPreprocessor parserVisitor = new ParsedQueryPreprocessor();
        VirtuosoSparqlFullTextSearchSerializerQueryVisitor serializerVisitor = new VirtuosoSparqlFullTextSearchSerializerQueryVisitor();
        SerializableParsedConstructQuery toSerialize = parserVisitor.transformToSerialize(parsedQuery);
        return serializerVisitor.serialize(toSerialize);
    }
