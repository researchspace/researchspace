...
export class ProfileSelectionComponent extends Component<Props, State> {
  ...
  private onChange(strVal: string, options: ReactSelectOptions<string>) {
    ...
    const selectedProfiles = <Profiles>this.props.profiles.filter((p) => _.includes(values, p.iri.value));
    this.props.onChange(selectedProfiles);
  }

  private prepareSelectData(props: Props) {
    const options = props.profiles
      .map((e) => {
        return {
          value: e.iri.value,
          label: e.label,
      ...
      .toJS();
    const selectedValues = props.selectedProfiles.map((e) => e.iri.value).toJS();
