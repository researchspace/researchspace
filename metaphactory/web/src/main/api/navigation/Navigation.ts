...
export function listen(cb: Listener) {
  ...
  return () => { _.pullAt(listeners, index); };
}
...
export function navigateToResource(
  ...
  return constructUrlForResource(iri, props, repository).flatMap(
    navigateToUrl
  ).toProperty();
}
...
export function constructUrlForResource(
  iri: Rdf.Iri, props: {} = {}, repository = 'default'
    ...
    listeners.map(
      listener => {
        if (isBeforeNavigationListener(listener)) {
          return Kefir.fromCallback<boolean>(
