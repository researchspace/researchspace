...
public class SparqlUtilTest {
    ...
    public void testGetTupleQueryResultWriterForAcceptedMediaTypes() {
        TupleQueryResultWriter writer = SparqlUtil.getTupleQueryResultWriterForAcceptedMediaTypes(System.out,
                Lists.newArrayList(MediaType.TEXT_HTML_TYPE, MediaType.APPLICATION_XML_TYPE,
    ...
    public void testGetTupleQueryResultWriterForAcceptedMediaTypesDefault() {
        TupleQueryResultWriter writer = SparqlUtil.getTupleQueryResultWriterForAcceptedMediaTypes(System.out,
                Lists.newArrayList(MediaType.TEXT_HTML_TYPE), TupleQueryResultFormat.JSON);
        assertTrue(writer instanceof SPARQLResultsJSONWriter);
    }

    @Test
    public void testGetRDFWriterForAcceptedMediaTypes() {
        RDFWriter writer = SparqlUtil.getRDFWriterForAcceptedMediaTypes(System.out,
                Lists.newArrayList(MediaType.TEXT_HTML_TYPE, MediaType.APPLICATION_XML_TYPE,
                        MediaType.APPLICATION_JSON_TYPE),
                RDFFormat.TURTLE);
    ...
    public void testGetRDFWriterForAcceptedMediaTypesDefault() {
        RDFWriter writer = SparqlUtil.getRDFWriterForAcceptedMediaTypes(System.out,
                Lists.newArrayList(MediaType.TEXT_HTML_TYPE, MediaType.APPLICATION_SVG_XML_TYPE), RDFFormat.TURTLE);
        /**
         * Since RDF4J 2.3 turtle writer is wrapped into a
         * {@link org.eclipse.rdf4j.rio.turtle.ArrangedWriter}, which is packaged
         * private. So effectively we can only test on the RDFFormat of the
         * ArrangedWriter object here
         */
    ...
    public void testReplaceAskWithSelect() {
        String askQuery = "PREFIX rdfs: <" + RDFS.NAMESPACE + "> ASK \t \n WHERE { ?x rdfs:label ?label . }";
        String selectQuery = SparqlUtil.transformAskToSelect(askQuery);
        Assert.assertEquals("PREFIX rdfs: <" + RDFS.NAMESPACE + "> SELECT * WHERE { ?x rdfs:label ?label . } LIMIT 1",
                selectQuery);
    }
    ...
    public void testReplaceAskWithSelectNoWhereNoSpace() {
        String askQuery = "PREFIX rdfs: <" + RDFS.NAMESPACE + "> ask{ ?x rdfs:label ?label . }";
        String selectQuery = SparqlUtil.transformAskToSelect(askQuery);
        Assert.assertEquals("PREFIX rdfs: <" + RDFS.NAMESPACE + "> SELECT * WHERE { ?x rdfs:label ?label . } LIMIT 1",
                selectQuery);
    }
    ...
    public void testReplaceAskWithSelectFail() {
        String askQuery = "PREFIX rdfs: <" + RDFS.NAMESPACE + "> SELECT * WHERE { ?x rdfs:label ?label . }";
        try {
