...
/**
 * Metaphactory specific security manager. Contains custom logics for Shiro realms,
...
public class MetaphactsSecurityManager extends DefaultWebSecurityManager {
     ...
     *             if the {@link ShiroTextRealm} can not be found i.e. has not
     *             been registered
     */
    public ShiroTextRealm getShiroTextRealm() throws IllegalStateException {
        DefaultSecurityManager manager = (DefaultSecurityManager) SecurityUtils
                .getSecurityManager();
        for (Realm realm : manager.getRealms()) {
            if (realm instanceof ShiroTextRealm)
                return (ShiroTextRealm) realm;
        }
        throw new IllegalStateException(ShiroTextRealm.class.getSimpleName()
                + " can not be found.");
    }
