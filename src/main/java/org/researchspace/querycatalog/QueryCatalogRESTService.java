 ...
 * published in a query catalog. In order to be callable via REST, the query
 * template must be exposed using a properties file stored in <i>config/qaas</i>
 * folder. Each file has a name <i>{SERVICE_ID}.prop</i> and contains the
 * following properties:
 * <ul>
 * <li><b>iri</b>(mandatory): the IRI of the SPIN SELECT query template stored
 * in the query catalog.</li>
 * <li><b>aclPermission</b>(optional): the Shiro permission string to check
 * whether the service can be executed. By default, the query is executable by
 * all users.</li>
 * <li><b>publisher</b>(optional): the IRI of the user who exposed the
 * service.</li>
 * <li><b>responseformat</b>(optional): the default MIME type to serialize query
 * results. It is used if no Accept headers were provided with the HTTP request.
 * Supported values: application/json, text/csv, and text/xml. Default:
 * application/json.</li>
 * <li><b>repository</b>(optional): id of the repository over which the query
 * will be executed. Default:
 * {@link RepositoryManager#DEFAULT_REPOSITORY_ID}.</li>
 * <li><b>disabled</b>(optional): a boolean value determining whether the
 * service is currently suppressed and is not accessible via the API. Default:
 * false.</li>
 * </ul>
 * 
 * 
...
public final class QueryCatalogRESTService {
    ...
    protected static Value interpretInputParameter(QueryArgument arg, String val) {
        if (arg == null) {
            throw new IllegalArgumentException("Query argument not provided for value " + val);
        }
        if (val == null) {
            throw new IllegalArgumentException("Parameter value for the argument " + arg.getPredicate() + " is null");
        }

        if (arg.getValueType() == null) {
            return VF.createLiteral(val);
        } else if (arg.getValueType().equals(RDFS.RESOURCE) || arg.getValueType().equals(XMLSchema.ANYURI)) {
     ...
     * Default constructor.
     * 
     * @param id                 The identifier of the REST service (properties file
     *                           name without the ".prop" extension)
     * @param configuration      configuration bound to the properties file.
     * @param queryTemplateCache A wrapper over the LDP API retrieving
     *                           {@link SelectQueryTemplate} objects.
     * @param repositoryManager  reference to the system repository manager.
     */
    public QueryCatalogRESTService(String id, PropertiesConfiguration configuration,
            QueryTemplateCache queryTemplateCache, RepositoryManager repositoryManager,
    ...
    /**
     * Runs the query with provided {@code arguments} over {@code connection} and
     * writes the results into {@code output}.
     * 
     * @param arguments a {@link MultivaluedMap} of arguments, which get interpreted
     *                  according to the datatypes defined in the query template.
     * @param output    an {@link OutputStream} to write the query results into.
     * @param mimeType  an {@link Optional} containing the desired mime type. If
     *                  empty or not suitable, the one returned by
     *                  {@link #getDefaultFormat()} will be used.
     */
    public void invoke(MultivaluedMap<String, String> arguments, OutputStream output, Optional<MediaType> mimeType)
            throws Exception {
        if (!checkPermission()) {
    ...
    /**
     * Returns the specific media type for returned results given the provided list
     * of accepted media types.
     * 
    ...
    public Optional<MediaType> determineMediaType(List<MediaType> acceptedMediaTypes) throws Exception {
        ...
        List<MediaType> allAcceptedMediaTypes = this.determineAcceptedMediaTypes(acceptedMediaTypes, queryTemplate);
    ...
    private void invokeInternal(QueryTemplate<?> queryTemplate, RepositoryConnection connection,
            MultivaluedMap<String, String> arguments, OutputStream output, List<MediaType> acceptedMediaTypes)
            throws IOException {
        if (queryTemplate instanceof SelectQueryTemplate) {
            invokeSelectQuery((SelectQueryTemplate) queryTemplate, connection, arguments, output, acceptedMediaTypes);
        } else if (queryTemplate instanceof ConstructQueryTemplate) {
            invokeConstructQuery((ConstructQueryTemplate) queryTemplate, connection, arguments, output,
                    acceptedMediaTypes);
        } else if (queryTemplate instanceof AskQueryTemplate) {
            invokeAskQuery((AskQueryTemplate) queryTemplate, connection, arguments, output, acceptedMediaTypes);
        } else if (queryTemplate instanceof UpdateQueryTemplate) {
            invokeUpdateQuery((UpdateQueryTemplate) queryTemplate, connection, arguments, output);
        } else {
            throw new IllegalArgumentException("Unsupported type of query in the query catalog REST API.");
        }
    }

    private void invokeSelectQuery(SelectQueryTemplate queryTemplate, RepositoryConnection connection,
            MultivaluedMap<String, String> arguments, OutputStream output, List<MediaType> acceptedMediaTypes) {
        SelectQuery query = queryTemplate.getQuery();
        TupleQuery tq = SparqlOperationBuilder.<TupleQuery>create(query.getQueryString(), TupleQuery.class)
                .setNamespaces(this.standardPrefixes).build(connection);
        bindArguments(tq, queryTemplate, arguments);
        TupleQueryResultHandler handler = SparqlUtil.getTupleQueryResultWriterForAcceptedMediaTypes(output,
                acceptedMediaTypes, TupleQueryResultFormat.JSON);
        tq.evaluate(handler);
    }

    private void invokeConstructQuery(ConstructQueryTemplate queryTemplate, RepositoryConnection connection,
            MultivaluedMap<String, String> arguments, OutputStream output, List<MediaType> acceptedMediaTypes) {
        ConstructQuery query = queryTemplate.getQuery();
        GraphQuery gq = SparqlOperationBuilder.<GraphQuery>create(query.getQueryString(), GraphQuery.class)
                .setNamespaces(this.standardPrefixes).build(connection);
        bindArguments(gq, queryTemplate, arguments);
        RDFWriter handler = SparqlUtil.getRDFWriterForAcceptedMediaTypes(output, acceptedMediaTypes, RDFFormat.TURTLE);
        gq.evaluate(handler);
    }

    private void invokeAskQuery(AskQueryTemplate queryTemplate, RepositoryConnection connection,
            MultivaluedMap<String, String> arguments, OutputStream output, List<MediaType> acceptedMediaTypes) {
        AskQuery query = queryTemplate.getQuery();
        BooleanQuery bq = SparqlOperationBuilder.<BooleanQuery>create(query.getQueryString(), BooleanQuery.class)
                .setNamespaces(this.standardPrefixes).build(connection);
        bindArguments(bq, queryTemplate, arguments);
        BooleanQueryResultWriter handler = SparqlUtil.getBooleanQueryResultWriterForAcceptedMediaTypes(output,
                acceptedMediaTypes, BooleanQueryResultFormat.JSON);
        handler.handleBoolean(bq.evaluate());
    }

    private void invokeUpdateQuery(UpdateQueryTemplate queryTemplate, RepositoryConnection connection,
            MultivaluedMap<String, String> arguments, OutputStream output) throws IOException {
        UpdateQuery query = queryTemplate.getQuery();
        Update uq = SparqlOperationBuilder.<Update>create(query.getQueryString(), Update.class)
                .setNamespaces(this.standardPrefixes).build(connection);
    ...
    private void bindArguments(Operation operation, QueryTemplate<?> queryTemplate,
        ...
        for (QueryArgument arg : queryTemplate.getArguments()) {
            if (arguments.containsKey(arg.getPredicate())) {
                ...
                Value val = interpretInputParameter(arg, vals.get(0));
                ...
                throw new IllegalArgumentException("No value given for a non-optional argument " + arg.getPredicate());
            }
        }
    }

    private List<MediaType> determineAcceptedMediaTypes(List<MediaType> acceptedMediaTypes,
        ...
        case SELECT:
            allMediaTypes.add(MediaType.valueOf(TupleQueryResultFormat.JSON.getDefaultMIMEType()));
            mimeTypesForQueryType.addAll(
                    SparqlUtil.getRegisteredMimeTypesForQueryType(TupleQueryResultWriterRegistry.getInstance()));
            break;
        case CONSTRUCT:
            allMediaTypes.add(MediaType.valueOf(RDFFormat.TURTLE.getDefaultMIMEType()));
            mimeTypesForQueryType
                    .addAll(SparqlUtil.getRegisteredMimeTypesForQueryType(RDFWriterRegistry.getInstance()));
            break;
        case ASK:
            allMediaTypes.add(MediaType.valueOf(BooleanQueryResultFormat.JSON.getDefaultMIMEType()));
            mimeTypesForQueryType.addAll(
                    SparqlUtil.getRegisteredMimeTypesForQueryType(BooleanQueryResultWriterRegistry.getInstance()));
            break;
        default:
        }
