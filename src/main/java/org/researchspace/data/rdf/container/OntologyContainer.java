...
/**
 * @author Diana Tanase <diana@kartography.org>
 */
@LDPR(iri = OntologyContainer.URI_STRING)
public class OntologyContainer extends DefaultLDPContainer {

    private static final Logger logger = LogManager.getLogger(OntologyContainer.class);

    public static final String URI_STRING = "http://www.researchspace.org/resource/system/ontologyContainer";
    public static final IRI IRI = vf.createIRI(URI_STRING);

    public OntologyContainer(IRI iri, MpRepositoryProvider repositoryProvider) {
    ...
    public void initialize() {
        if (!getReadConnection().hasOutgoingStatements(this.getResourceIRI())) {
            logger.debug("Initializing container with basic metadata.");
            LinkedHashModel m = new LinkedHashModel();
            m.add(vf.createStatement(IRI, RDF.TYPE, LDP.Container));
            m.add(vf.createStatement(IRI, RDF.TYPE, LDP.Resource));
            m.add(vf.createStatement(IRI, RDFS.LABEL, vf.createLiteral("Ontology Container")));
            m.add(vf.createStatement(IRI, RDFS.COMMENT, vf.createLiteral(
                    "Container to store resources of rdf:type owl:Ontology")));
            try {
