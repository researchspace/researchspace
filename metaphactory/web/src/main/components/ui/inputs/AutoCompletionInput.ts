...
export class AutoCompletionInput extends Component<AutoCompletionInputProps, {}> {
  ...
  render() {
    const result = createElement(AbstractAutoCompletionInput,
      assign({
        ref: (comp) => { this.autoCompletion = comp; },
      }, this.props, {
  ...
  private executeQuery =
      ...
      (token: string, tokenVariable: string) => {
        ...
        const luceneQuery = SparqlUtil.makeLuceneQuery(token);
        const queryWithToken = SparqlClient.setBindings(
          parsedQuery, {[SEARCH_INPUT_VARIABLE]: luceneQuery}
        );
  ...
  private replaceTokenAndParseQuery = (
    queryString: string, tokenVariable: string, token: string
  ) => {
    let q = queryString;
    if (queryString.indexOf(SEARCH_INPUT_VARIABLE) === -1) {
      ...
      q = queryString.replace(
        new RegExp('\\\?' + tokenVariable), token
      );
    }

    return SparqlUtil.parseQuerySync(q);
  }
}
