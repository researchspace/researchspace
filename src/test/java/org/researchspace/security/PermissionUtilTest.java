...
public class PermissionUtilTest extends AbstractIntegrationTest {
    ...
    public static void validatePermissions(File file) throws Exception {
        try (InputStream stream = new FileInputStream(file)) {
            ...
            for (String commaSeperatedPermissions : rolesSection.values()) {
                ...
                for (Permission permission : PermissionUtils.resolveDelimitedPermissions(commaSeperatedPermissions,
                        permissionResolver)) {
                    Assert.assertTrue("Permission in default shiro.ini should be valid",
                            PermissionUtil.isPermissionValid(permission.toString()));
                }
            }
        }
    }
    ...
    public void testAdmin() throws Exception {
        ...
        Assert.assertTrue(
                PermissionUtil.hasSparqlPermission(SparqlOperation.SELECT, RepositoryManager.DEFAULT_REPOSITORY_ID));
        Assert.assertTrue(
                PermissionUtil.hasSparqlPermission(SparqlOperation.ASK, RepositoryManager.DEFAULT_REPOSITORY_ID));
        Assert.assertTrue(
                PermissionUtil.hasSparqlPermission(SparqlOperation.CONSTRUCT, RepositoryManager.DEFAULT_REPOSITORY_ID));
        Assert.assertTrue(
                PermissionUtil.hasSparqlPermission(SparqlOperation.DESCRIBE, RepositoryManager.DEFAULT_REPOSITORY_ID));
        Assert.assertTrue(
                PermissionUtil.hasSparqlPermission(SparqlOperation.UPDATE, RepositoryManager.DEFAULT_REPOSITORY_ID));

        Assert.assertTrue(
                PermissionUtil.hasSparqlPermission(SparqlOperation.SELECT, RepositoryManager.ASSET_REPOSITORY_ID));
        Assert.assertTrue(
                PermissionUtil.hasSparqlPermission(SparqlOperation.ASK, RepositoryManager.ASSET_REPOSITORY_ID));
        Assert.assertTrue(
                PermissionUtil.hasSparqlPermission(SparqlOperation.CONSTRUCT, RepositoryManager.ASSET_REPOSITORY_ID));
        Assert.assertTrue(
                PermissionUtil.hasSparqlPermission(SparqlOperation.DESCRIBE, RepositoryManager.ASSET_REPOSITORY_ID));
        Assert.assertTrue(
                PermissionUtil.hasSparqlPermission(SparqlOperation.UPDATE, RepositoryManager.ASSET_REPOSITORY_ID));
    }

    @Test
    @SubjectAware(username = "guest", password = "guest", configuration = sparqlPermissionShiroFile)
    ...
    @Test
    @SubjectAware(configuration = sparqlPermissionShiroFile)
