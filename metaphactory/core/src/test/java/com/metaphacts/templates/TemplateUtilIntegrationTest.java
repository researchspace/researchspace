...
public class TemplateUtilIntegrationTest extends AbstractRepositoryBackedIntegrationTest {
    ...
    public void setup() throws Exception {
        this.templateStorage = new SimpleFileTemplateStorage(testFolder.newFolder());
        NamespaceRegistry ns = namespaceRule.getNamespaceRegistry();
        TemplateLoader loader = new FileTemplateLoader(templateStorage, ns);
        this.handlebars = new MetaphactsHandlebars(loader, new HandlebarsHelperRegistry(labelCache, ns, repositoryRule.getRepositoryManager()));
    }
    ...
    public void getTemplateSourceTest() throws Exception{
        ...
        templateStorage.storeNewRevision(FOAF.PERSON, content);
    ...
    public void findFirstExistingTemplateTest() throws Exception{
        templateStorage.storeNewRevision(FOAF.PERSON, "This is the person template");
        templateStorage.storeNewRevision(FOAF.AGENT, "This is the agent template");
    ...
    public void getRdfTemplateIncludeIdentifiersTest() throws Exception{
        ...
        config.getUiConfig().setParameter("templateIncludeQuery", "SELECT ?type WHERE { ?? a ?type }");
    ...
    public void getRdfTemplateIncludeIdentifiersOrderTest() throws Exception{
        config.getUiConfig().setParameter("templateIncludeQuery", "SELECT ?type WHERE { VALUES(?type) {(<"+FOAF.PERSON.stringValue()+">) (<"+FOAF.AGENT.stringValue()+">)  (<"+SKOS.CONCEPT.stringValue()+">)} }");
    ...
    public void getPageDifferentFromContextValue() throws Exception{
        ...
        templateStorage.storeNewRevision(page, "This the page testpage123, but the context is: [[this]]");
        assertEquals(
    ...
    private TemplateContext context(IRI iri){
        TemplateContext context = new TemplateContext(iri, this.repositoryRule.getRepository(), null);
