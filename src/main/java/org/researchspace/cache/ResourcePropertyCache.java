...
/**
 * Cache with extraction logic for batched access to some property of a
 * resource. (?subject ?predicate ?object) triple corresponds to (resource
...
public abstract class ResourcePropertyCache<Key, Property> implements PlatformCache {
    ...
    private void initializeCache(Repository repository) {
        ...
        repositoryMap.put(repository, createCacheBuilder().build(new CacheLoader<Key, Optional<Property>>() {
            @Override
    ...
    public void invalidate(Set<IRI> iris) {
        repositoryMap.values().forEach(cache -> {
            List<Key> keysToInvalidate = cache.asMap().keySet().stream().filter(key -> iris.contains(keyToIri(key)))
    ...
    /**
     * Constructs a query that extracts the property values for all properties in
     * the preferredProperties input list and the given IRIs as input.
     *
     ...
     * </code>
     *
     * , i.e. the ?pi variables denote by their index the position of the respective
     * property in the preferredProperties list (e.g., ?p0 will contain bindings for
     * preferredProperties.get(0)), whereas the values are injected via VALUES
     * clause.
     *
     ...
     */
    protected static String constructPropertyQuery(Iterable<? extends IRI> iris,
            List<PropertyPattern> preferredProperties) {

        // build query value injection clause
        StringBuilder valuesClauseBuilder = new StringBuilder().append("VALUES (?subject) { ");
        for (IRI iri : iris) {
     ...
     * (1) the outer list represents the predicate index and (2) the inner list
     * contains the property values for this predicate index
     *
     * The extraction is done efficient in a single pass (linear time & space w.r.t.
     * result size).
     *
     * @param repository      the repo over which to execute the query
     * @param queryString     the query string (constructed by
     *                        {@link #constructPropertyQuery(Iterable, List)})
     * @param propertyCount   the count of properties in query
     * @param extractProperty the function to get property value from Sparql query
     *                        result binding value
     * @return the list in which we store the result
     */
    protected static <Property> Map<IRI, List<List<Property>>> queryAndExtractProperties(Repository repository,
        ...
        final SparqlOperationBuilder<TupleQuery> op = SparqlOperationBuilder.create(queryString, TupleQuery.class);
