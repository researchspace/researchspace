...
public class MpSparqlServiceRegistry {
    ...
    private static StoragePath getConfigObjectId(String serviceID) {
        return SERVICES_OBJECT_PREFIX.resolve(normalizeServiceConfigId(serviceID)).addExtension(".ttl");
    }
    ...
    protected void init() throws IOException {
        List<ObjectRecord> turtleFiles = platformStorage.findAll(SERVICES_OBJECT_PREFIX).values().stream()
            ...
            try (InputStream content = configFile.getLocation().readContent()) {
                ...
                if (!optional.isPresent()) {
                    throw new RepositoryConfigException(
                            "Object at path \"" + configFile.getPath() + "\" does not contain a service descriptor");
                }
    ...
    public void addOrUpdateServiceConfig(String serviceId, String configText, @Nullable ServiceDescriptor existing)
        ...
        try {
            ...
            String msg = "Could not parse the submitted service descriptor object at path \"" + objectId + "\": "
                    + e.getMessage();
            throw new IllegalArgumentException(msg, e);
        }

        Optional<IRI> optSubjectIri = Models
                .subjectIRI(serviceDescriptorModel.filter(null, RDF.TYPE, MpRepositoryVocabulary.SERVICE_TYPE));
        ...
        try {
            ObjectStorage targetStorage = platformStorage.getStorage(PlatformStorage.DEVELOPMENT_RUNTIME_STORAGE_KEY);

            targetStorage.appendObject(objectId, platformStorage.getDefaultMetadata(),
            ...
            throw new StorageException("Could not write the repository configuration object at path \"" + objectId
                    + "\": " + e.getMessage(), e);
        }
    }
    ...
    public String getDescriptorText(String id) throws IOException {
        FindResult serviceConfig = platformStorage.findObject(getConfigObjectId(id))
                .orElseThrow(() -> new IOException("Descriptor with ID '" + id + "' was not found in the storage"));

        try {
            return StorageUtils.readTextContent(serviceConfig.getRecord());
        } catch (IOException e) {
            throw new IOException(
                    "Error while reading repository configuration file from file system: " + e.getMessage(), e);
        }
    }
    ...
    public void deleteDescriptor(String id) throws IOException {
        ...
        Optional<FindResult> optServiceConfig = platformStorage.findObject(getConfigObjectId(id));
        ...
        if (optServiceConfig.isPresent()) {
            ...
            storage.deleteObject(res.getRecord().getPath(), platformStorage.getDefaultMetadata());
        }
