...
import org.researchspace.federation.repository.evaluation.MergeByIndexInsertBindingSetCursor;
...
/**
 * Extended implementation of {@link OwnedTupleExpr}. Replaced instead of
 * inheritance to avoid extensive use of reflection to access private fields and
 * methods.
 * 
...
public class MpOwnedTupleExpr extends UnaryTupleOperator {
    ...
    public CloseableIteration<BindingSet, QueryEvaluationException> evaluate(Dataset dataset, List<BindingSet> bindings,
            Set<String> boundVars) throws QueryEvaluationException {
        CloseableIteration<BindingSet, QueryEvaluationException> rval = null;
        if (queryAsString != null) {
            try {
                if (preparedQuery instanceof TupleQuery) {
                    ...
                    throw new UnsupportedOperationException("Unsupported query type: " + this.preparedQuery);
                }
            } catch (Exception e) {
