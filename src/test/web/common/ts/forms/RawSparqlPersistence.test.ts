...
describe('RawSparqlPersistence', () => {
  const definition = normalizeFieldDefinition({
    ...
    maxOccurs: 3,
    selectPattern:
      'prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> ' + 'SELECT ?value WHERE {$subject rdfs:label ?value}',
    insertPattern:
      'prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> ' + 'INSERT { $subject rdfs:label $value } WHERE {}',
    deletePattern:
  ...
  const values = Immutable.List<FieldValue>().push(FieldValue.fromLabeled({ value: Rdf.literal('testValue') }));
